...
PAYONE still returns to the configured backend callback. After checking payment status, the provider resolves the stored path against the configured cabinet origin. The host must have `doRedirect: true` to issue the browser redirect. Missing or invalid paths use the configured `frontRedirectUrl` fallback. For every payment outcome, a standard `/tasks/{taskId}` destination (including a saved task-root path) is extended with the top-level step from `paymentControlPath` (for example, `paymentInfo.properties.paymentControl` returns to `/tasks/{taskId}/paymentInfo`). Explicit step URLs and custom routes are preserved. Callback-supplied return paths are ignored. No frontend origin is trusted, and the return path is not sent to PAYONE.
