...
interface LanguageSelectorPalette {
  background?: string;
...
type AppTheme = Theme & {
  ...
  borderColor?: string;
  outlineColor?: string;
  languageSelector?: LanguageSelectorPalette;
};

const useStyles = makeStyles((theme: AppTheme) => {
  const getPalette = (darkTheme?: boolean) => {
    ...
    const baseOverrides: LanguageSelectorPalette = theme?.languageSelector || {};
  ...
  return {
    ...
    group: ({ darkTheme }: { darkTheme?: boolean }) => {
    ...
    select: ({ darkTheme }: { darkTheme?: boolean }) => {
      const palette = getPalette(darkTheme);
...
export const LanguageSelector = ({ darkTheme = false }: { darkTheme?: boolean }) => {
  ...
  const localization = useSelector((state) => (state as { app?: { localization?: { code: string }[] } })?.app?.localization || []);
  ...
  const handleChange = (_: unknown, next: string) => {
    ...
    dispatch(setCurrentLanguage(next) as never);
