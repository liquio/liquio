...
interface WindowFetchEvent extends Event {
  request: Request;
...
export function register(config?: ServiceWorkerConfig): void {
  if ('serviceWorker' in navigator) {
    ...
    const publicOrigin = new URL(publicUrl || '/', window.location.href);
    if (publicOrigin.origin !== window.location.origin) {
    ...
    window.addEventListener('fetch', (rawEvent) => {
      const event = rawEvent as unknown as WindowFetchEvent;
    ...
    window.addEventListener('beforeinstallprompt', (rawEvent) => {
      const e = rawEvent as InstallPromptEvent;
      e.preventDefault();
