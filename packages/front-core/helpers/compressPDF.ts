...
pdfjsLib.GlobalWorkerOptions.workerSrc = new URL('js/pdfjsLib/pdf.worker.min.mjs', `${window.location.origin}`).toString();

interface CompressPdfParams {
  attach: File;
...
async function compressPDF({ attach, outputQuality }: CompressPdfParams): Promise<File | Blob> {
  return new Promise((resolve) => {
    const fileReader = new FileReader();
    const resultDocs: RenderedPage[] = [];
    ...
    fileReader.onload = async function () {
      try {
        ...
        const blob = base64ToBlob(mergedDoc.output('datauristring')) as Blob & Record<string, unknown>;

        blob.labels = (attach as unknown as Record<string, unknown>)?.labels;
        blob.path = (attach as unknown as Record<string, unknown>)?.path;
        blob.lastModified = attach?.lastModified;
        blob.lastModifiedDate = (attach as unknown as Record<string, unknown>)?.lastModifiedDate;
