import { parseLoginResponse } from 'core/helpers/authContracts';
...
export const getQueryLangParam = () => {
  ...
  const params: { lang?: string } = searchString
  ...
  if (langExists) {
    ...
    storage.setItem('lang', params.lang as string);
    return params.lang;
  }
...
export const setCurrentLanguage = (code: string) => (dispatch: Dispatch) => {
...
export const requestUserInfo = () => (dispatch: Dispatch) => {
  ...
  return api.get(`${AUTH_URL}/me${queryString}`, REQUEST_USER_INFO, dispatch).then((authResult) => {
...
export const requestUnits = () => (dispatch: Dispatch) =>
...
export const requestUserSettings = () => (dispatch: Dispatch) =>
...
export const requestTestCode = () => () => {
  const config = getConfig() as unknown as { testAuth: { body: unknown; url: string; headers?: Record<string, string> } };
  const {
    testAuth,
    testAuth: { body, url }
  } = config;
...
export const loginByCode = (code: string, state?: string) => (dispatch: Dispatch) =>
  api
...
export const requestAuth = (code: string, state?: string) => async (dispatch: Dispatch) => {
...
export const updateUserInfo = (userInfo: unknown) => async (dispatch: Dispatch) =>
...
export const isRole = (check: string) => {
  const {
    auth: { info }
  } = store.getState() || {};
  if (!info) {
  ...
  // Real runtime usage always treats this as an array; the union with
  // Record<string, string> in its type never actually reaches here.
  return (courtIdUserScopes as string[]).includes(check);
};
...
export const getUserSuggestions = (searchString: string) => (dispatch: Dispatch) => {
  return api
    ...
    .then((result) => (result as { users: unknown }).users);
};
...
export const checkPhoneExists = (phone: string) => (dispatch: Dispatch) =>
  api
...
export const setAuthMode = (mode: unknown) => (dispatch: Dispatch) => {
  return api.post(`${USERS_URL}/two_factor_auth`, mode, REQUEST_AUTH_MODE, dispatch);
};

export const sendSMSCode = (phone: string) => (dispatch: Dispatch) =>
  api
...
export const verifySMSCode = (phone: string, code: string) => (dispatch: Dispatch) =>
  api
...
export const sendEmailCode = (email: string) => (dispatch: Dispatch) =>
...
export const verifyEmailCode = (email: string, code: string) => (dispatch: Dispatch) =>
  api
...
export const onlyVerifyEmailCode = (email: string, code: string) => (dispatch: Dispatch) =>
  api
    .post(
...
export const searchUser = (data: unknown) => (dispatch: Dispatch) => {
...
export const generateTOTP = () => (dispatch: Dispatch) =>
  api.get('totp/generate', 'GENERATE_TOTP', dispatch).catch((error) => {
    Sentry.captureException(error);
    return error;
  });

export const enableTOTP = (secret: string, code: string) => (dispatch: Dispatch) =>
  api.post('totp/enable', { secret, code }, 'ENABLE_TOTP', dispatch).catch((error) => {
    Sentry.captureException(error);
    return error;
  });

export const disableTOTP = (code: string) => (dispatch: Dispatch) =>
  api.post('totp/disable', { code }, 'DISABLE_TOTP', dispatch).catch((error) => {
    Sentry.captureException(error);
    return error;
  });
export const setPassword = (userId: string | number, body: unknown) => (dispatch: Dispatch) =>
  api.post(`users/${userId}/set-password`, body, 'SET_PASSWORD', dispatch);

export const changePassword = (body: unknown) => (dispatch: Dispatch) =>
  api.post('local/change_password', body, 'CHANGE_PASSWORD', dispatch).then(async (data) => {
    await requestUserInfo()(dispatch);
    return data;
  });

export const createUser = (body: unknown) => (dispatch: Dispatch) =>
  api.post('users/create_local', body, 'CREATE_USER', dispatch);

export const enable2FA = (userId: string | number) => (dispatch: Dispatch) =>
  api.post(`users/${userId}/enforce-2fa`, {}, 'TOGGLE_2FA', dispatch);

export const disable2FA = (userId: string | number) => (dispatch: Dispatch) =>
