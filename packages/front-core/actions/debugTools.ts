...
export const setCheckHiddenFunc = (taskId: string | number, func: unknown) => ({
  type: SET_CHECK_HIDDEN_FUNC,
  payload: { taskId, func }
});

export const setCheckValidFunc = (taskId: string | number, func: unknown) => ({
  type: SET_CHECK_VALID_FUNC,
  payload: { taskId, func }
});

export const setPopupData = (data: unknown) => ({
  type: SET_POPUP_DATA,
  payload: data
});

export const setCustomInterfaceData = (data: unknown) => ({
  type: SET_CUSTOM_INTERFACE_DATA,
  payload: data
});

export const getMocksKeysByUser = (options: string) => (dispatch: Dispatch) =>
  api
