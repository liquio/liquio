...
function keyToUint8Array(input: Record<string, number>) {
...
interface EDSSignVerifyProps {
  classes: Record<string, string>;
  t: (key: string) => string;
}

const EDSSignVerify = ({ classes, t }: EDSSignVerifyProps) => {
  ...
  const getSignData = async () => {
    try {
      ...
      const {
        ...
        data: singData,
        timeInfo,
      } = (await signer.execute('VerifyDataInternal', sign)) as unknown as {
        ownerInfo: unknown;
...
const styled = withStyles(styles)(EDSSignVerify as never);
