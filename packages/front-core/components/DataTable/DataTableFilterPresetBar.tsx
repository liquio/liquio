...
import AddNewPresetDialogRaw from 'components/DataTable/components/AddNewPresetDialog';
import diff from 'helpers/diff';

const AddNewPresetDialog = AddNewPresetDialogRaw as unknown as React.ComponentType<Record<string, unknown>>;
...
interface DataTableFilterPresetBarProps {
  ...
  actions: {
    ...
    onFilterPresetDelete: (index: number) => void;
    onFilterPresetAdd: (preset: Preset) => void;
  };
...
const DataTableFilterPresetBar = ({
  ...
  const allowedFilters = Object.keys(filters)
    ...
    .reduce((acc, filterKey) => ({ ...acc, [filterKey]: filters[filterKey] }), {} as Record<string, unknown>);
  ...
  return (
    <Toolbar disableGutters={true}>
      {presets.map((preset, index) => (
        <Chip
          key={index}
          {...({ variant: index === activePreset ? 'default' : 'outlined' } as unknown as Record<string, unknown>)}
          size="small"
          label={preset.name}
      ...
      <AddNewPresetDialog
        ...
        handleAddPreset={(newPresetName: string) => {
          setOpen(false);
...
const styled = withStyles(styles)(DataTableFilterPresetBar as never);
