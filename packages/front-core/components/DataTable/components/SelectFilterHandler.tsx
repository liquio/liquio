...
type AppTheme = Theme & {
  ...
  header?: { background?: string; textColor?: string; leftTitle?: { color?: string } };
  listHover?: string;
...
const styles = (theme: AppTheme) => ({
  ...
  darkThemeSelect: {
    // Preserved as-is: this reads the browser's global `navigator` object,
    // which has no `fillIcon` property — always `undefined` at runtime
    // (a real, pre-existing dead style, not something introduced here).
    color: (navigator as unknown as Record<string, unknown>).fillIcon as string,
...
interface SelectFilterHandlerState {
  search: string;
}
...
class SelectFilterHandler extends FilterHandler {
  ...
  handleChange = ({ target: { value } }: { target: { value: unknown } }) => {
    const { onChange } = this.props;
    onChange?.(value);
  };

  renderIcon = () => {
    const { classes, darkTheme, IconComponent } = this.props as FilterHandlerProps & {
  ...
  renderChip = () => {
    ...
    const valueSearch = isNaN(value as number) ? value : Number(value);
    const selectedOption = (options || []).filter((el) => el.id === valueSearch);
    const chipValue = selectedOption.length ? selectedOption[0].name : (value as string);

    return [name, useOwnNames ? chipValue : t?.(chipValue)].join(': ');
  };

  handleFilter = ({ target: { value: search } }: { target: { value: string } }) =>
    this.setState({
      search
    });
  ...
  filterList = () => {
    ...
    const { search } = this.state as unknown as SelectFilterHandlerState;
  ...
  renderListItemText = (option: { id: string | number; name: string }) => {
  ...
  renderHandler = () => {
    const { classes, t, value, useOwnNames, darkTheme, listDisplay, searchField, placeholder } = this
      .props as FilterHandlerProps & { classes: Record<string, string> };
    const { search } = this.state as unknown as SelectFilterHandlerState;
...
const styled = withStyles(styles)(SelectFilterHandler as never);
export default translate('SelectFilterHandler')(styled as never) as unknown as React.ComponentType<
