...
export const RunProcessButton = ({ workflowTemplateId, taskTemplateId }: RunProcessButtonProps) => {
  ...
  const handleClick = useCallback(() => {
    ...
    const redirectUrl = new URL(`/tasks/create/${workflowTemplateId}/${taskTemplateId}`, config.cabinetUrl);
    window.open(redirectUrl.toString(), '_blank');
  }, [config.cabinetUrl, workflowTemplateId, taskTemplateId]);

  return (
    <Tooltip title={t('RunProcess')}>
      <IconButton size="large" onClick={handleClick}><PlayArrowIcon /></IconButton>
    </Tooltip>
  );
};
