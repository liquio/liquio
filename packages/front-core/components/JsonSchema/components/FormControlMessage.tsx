...
interface FormControlMessageProps {
  children: ReactNode;
  size?: 'small' | 'inherit' | 'large' | 'medium';
  message: { type?: MessageType; title?: ReactNode; text?: ReactNode };
}

const FormControlMessage = ({ children, size, message: { type = 'warning', title, text } }: FormControlMessageProps) => {
  const classes = useStyles();
  const [anchorEl, setAnchorEl] = React.useState<Element | null>(null);
  const { Icon, color } = MESSAGE_TYPES[type] || MESSAGE_TYPES['warning'];

  const handlePopoverOpen = (event: React.MouseEvent<SVGSVGElement>) => setAnchorEl(event.currentTarget);
