...
const GroupedElementList = ({
  ...
  const [openControl, setOpenControl] = React.useState(
    storage.getItem('openControl') === 'true',
  );
  ...
  const sortedGroups = Object.keys(snippetGroups).map((group) => {
    return {
      ...
      sortIndex:
        (sortGroups || []).findIndex((element) => element === group) || 0,
    };
  });
      ...
      {sortedGroups.map(({ name: group, elements }, index) => {
        const filteredSnippets = elements
          ...
          .filter((element) => {
            ...
            const codeMatch = JSON.stringify(element)
              .toLowerCase()
              .includes(search.toLowerCase());
