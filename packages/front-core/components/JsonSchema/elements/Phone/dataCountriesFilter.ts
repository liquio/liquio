...
type RawCountry = [string, string[], string, string, string?, (number | string)?, (string[] | string)?, number?];
...
export interface CountryItem {
  ...
  format?: string;
  hasAreaCodes?: string[] | string;
  length?: number;
  isAreaCode?: boolean;
  formattedNumber?: string;
}

const rawAllCountries: RawCountry[] = [
...
function addCountryCode(iso2: string, dialCode: string, priority?: unknown) {
  if (!(dialCode in allCountryCodes)) {
    allCountryCodes[dialCode] = [];
  }
  const index = (priority as number) || 0;
  allCountryCodes[dialCode][index] = iso2;
}

const allCountries: CountryItem[] = ([] as CountryItem[]).concat(
  ...rawAllCountries.map((country) => {
    ...
    const areaItems: CountryItem[] = [];

    if (countryItem.hasAreaCodes) {
      (areaCodes as string[]).forEach((areaCode) => {
