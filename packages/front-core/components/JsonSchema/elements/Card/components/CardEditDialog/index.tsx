...
interface CardEditDialogProps {
  recoverData: unknown;
  schema: JsonSchemaNode & { description?: string; saveText?: string };
  value?: { open?: boolean };
...
const CardEditDialog = ({
  ...
  const handleClose = React.useCallback(() => {
    if (diff(props.rootDocument.data, recoverData)) {
      objectPath.set(
        recoverData,
        ([props.stepName] as Array<string | number>).concat(props.path, 'open'),
        false,
      );
      return props.actions.setValues(recoverData);
    }
    (props.onChange.bind(null, 'open') as (arg: unknown) => void)(new ChangeEvent(false, true));
  }, [
    props.onChange,
    props.actions,
