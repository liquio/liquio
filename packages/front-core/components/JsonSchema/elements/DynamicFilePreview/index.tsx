...
interface DynamicFilePreviewProps extends WithStyles<typeof styles> {
...
const DynamicFilePreview = (props: DynamicFilePreviewProps) => {
  const {
    rootDocument,
    dataPath,
    description,
  ...
  const showPreviewDialog = React.useCallback((item: DynamicFile) => {
    setShowPreview(item.url as string);
    setFileName(item?.name as string);
        ...
        >
          <TableContainer>
            <Table>
              ...
              <TableBody>
                {((files || []) as DynamicFile[]).map((file, index) => (
                  <TableRow key={index}>
                    <TableCell align={'left'} padding={'none'}>
                      {file.name}
                    </TableCell>
