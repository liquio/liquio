...
import SuccessMessageUntyped from 'modules/tasks/pages/Task/components/SuccessMessage';
...
const SuccessMessage = SuccessMessageUntyped as unknown as React.ComponentType<Record<string, unknown>>;

// validateAppleSession is only exported by cabinet-front's application/actions/task;
// admin-front's copy lacks it, so it is resolved dynamically to keep this file shared between both apps.
const validateAppleSession = (taskActions as unknown as Record<string, (...args: unknown[]) => (dispatch: Dispatch) => Promise<unknown>>).validateAppleSession;
...
interface PaymentWidgetProps extends WithStyles<typeof styles> {
  ...
  error?: unknown;
  required?: boolean;
  onChange: (name: string, val: { data?: unknown; [key: string]: unknown }) => void;
  recipients: Array<{ amount: string }> | { amount: string };
  path: Array<string | number>;
  rootDocument: { id?: string | number; data: Record<string, unknown> };
  importActions: {
    getPaymentInfo: (...args: unknown[]) => Promise<unknown>;
    getPaymentStatus: (...args: unknown[]) => Promise<unknown>;
    loadTask: (...args: unknown[]) => Promise<unknown>;
    addMessage: (message: unknown) => void;
    validateAppleSession: (...args: unknown[]) => Promise<unknown>;
  };
  paymentControlPath: string;
  taskId: string;
  stepName: string;
  items?: { properties: Record<string, unknown> } | null;
  ...
  handlePrevStep: () => void;
  actions: {
    validateStep: (arg: null, options: { ignorePaymentControl: boolean }) => Promise<boolean>;
    validateDocument: (documentId: unknown) => Promise<unknown>;
...
interface PaymentWidgetState {
  ...
  inited: boolean;
  isPending: boolean;
  calculatedHistory: unknown[] | null;
  processed: Array<{ status?: { isSuccess?: boolean; isPending?: boolean }; extraData?: unknown }> | null;
  extraData: unknown;
  messageShown: boolean;
  validateErrors: ValidateErrorEntry[];
  anotherPaymentWidgetIsBusy: boolean;
  loopRequest: number[];
  widget: boolean;
  applepay: boolean;
  gpay: boolean;
...
class PaymentWidget extends React.Component<PaymentWidgetProps, PaymentWidgetState> {
  static defaultProps = {
    hidden: false,
    description: null,
    sample: null,
  ...
  gpayMerchantId: unknown;

  gpayMerchantName: unknown;
  ...
  triggerTokenizer: React.RefObject<HTMLButtonElement | null>;

  queue: ReturnType<typeof queue>;

  queueAppleGoogle: ReturnType<typeof queue>;

  broadcastChannel: BroadcastChannel;

  widget?: UGBWidgetInstance;
  ...
  constructor(props: PaymentWidgetProps) {
    ...
    const { environment } = (application as { environment?: unknown }) || {};
    ...
    this.requestDelay = (paymentCheckDelay as number) || 3000;
    ...
    this.triggerTokenizer = React.createRef<HTMLButtonElement>();
    this.queue = queue({ autostart: true, concurrency: 1 });
    this.queueAppleGoogle = queue({ autostart: true, concurrency: 1 });

    this.broadcastChannel = new BroadcastChannel(
      ([props.rootDocument?.id] as Array<string | number | undefined>).concat(props.path || []).join()
    );
  ...
  handleChange = async (name: string, val: { data?: unknown; [key: string]: unknown }) => {
        ...
        {Object.keys(items.properties).map((key) => (
          <div key={key}>
            <SchemaForm
              ...
              stepName={stepName}
              errors={errors}
              schema={(items.properties as Record<string, unknown>)[key]}
  ...
  showStatus = ({ isSuccess, isPending, extraData }: { isSuccess: boolean; isPending: boolean; extraData?: unknown }) => {
    ...
    if (!isSuccess && !isPending) {
      this.validateControl();
      const errorMessage = (data: { status_description?: string } | undefined) => {
        if (
          data &&
      ...
      importActions.addMessage(new Message(errorMessage(extraData as { status_description?: string } | undefined), 'error'));
    }
  ...
  checkPaymentStatus = async ({ silent, isReload }: { silent: boolean; isReload?: boolean }) => {
    const {
      taskId,
      importActions,
      rootDocument: { id }
  ...
  onToken = async (tokenData: { token?: string } | null | undefined) => {
    const {
      importActions,
      rootDocument: { id },
      paymentControlPath
  ...
  getControlPath = () => {
    ...
    const controlPath = ([stepName] as Array<string | number>).concat(path).join('.');
    return controlPath;
  };
  ...
  validateDocument = async () => {
    ...
    const validateDocument = await actions.validateDocument(task?.documentId) as (Error & { details?: Array<{ dataPath: string }> }) | unknown;

    if (validateDocument instanceof Error) {
      const controlPath = this.getControlPath();

      const errors = ((validateDocument as { details?: Array<{ dataPath: string }> })?.details || []).filter(
  ...
  validateControl = async () => {
    ...
    const errors = validateData(virtualizedValue, schema as never, rootDocument?.data);
    ...
    if (errors.length) {
      this.setState({
        ...
        validateErrors: [
          {
            errorText: errors.map((err) => (err as unknown as { errorText: string }).errorText).join(', ')
          }
        ]
      });
    }
  ...
  renderFinalScreen = () => {
    ...
    return (
      <Dialog open={true} TransitionComponent={'div' as never} aria-labelledby="dialog">
        <div id="dialog">
          <SuccessMessage {...(jsonSchema as Record<string, unknown>)} taskId={taskId} rootPath={rootPath} task={task} />
        </div>
      </Dialog>
    );
  };

  parseResult = async ({ result, isUserAction, silent, isReload }: { result: unknown; isUserAction?: boolean; silent?: boolean; isReload?: boolean }) => {
    if (!result) return;

    const controlPath = this.getControlPath();

    const paymentInfo = objectPath.get((result as { data?: unknown }).data, controlPath) as
      | { processed?: PaymentWidgetState['processed']; calculated?: unknown; calculatedHistory?: unknown[]; paymentRequestData?: { url: string } }
      | undefined;
    ...
    this.setState({ calculatedHistory: calculatedHistory || null });
    ...
    } = calculated as { extraData: { user_action_required?: boolean; user_action_url?: string } };

    if (user_action_required && isUserAction) {
      window.location.href = user_action_url as string;
      return;
    }
  ...
  initWidgetEvents = () => {
    ...
    (this.widget as UGBWidgetInstance)
      .onReady()
      .then(() => {
        ...
        current.addEventListener('click', () => {
          (this.widget as UGBWidgetInstance).validateForm().then(async (isValid) => {
            this.broadcastChannel.postMessage(
              JSON.stringify({
                type: 'paymentState',
                payload: {
            ...
            isValid && (this.widget as UGBWidgetInstance).formSubmit();
          });
        });
      })
      .catch((e: { detail: { id: string; message: string } }) => {
  ...
  handleRadioChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    this.setState({
      ...
      privat24: false,
      [value]: checked
    } as unknown as PaymentWidgetState);
  };
  ...
  initPayment = async () => {
    ...
    if (gpay) {
      const paymentsClient = new (window.google as NonNullable<Window['google']>).payments.api.PaymentsClient({
      ...
      applePaySession.onpaymentauthorized = async (event) => {
        ...
        applePaySession.completePayment((window.ApplePaySession as ApplePaySessionConstructor).STATUS_SUCCESS);
  ...
  render = () => {
    ...
    const isFinalScreen =
      isSuccess || (processed && processed.find(({ status }) => status?.isSuccess));
        ...
        {!loading && isUpdateble && !validateErrors.length ? (
          <Fade in={true}>
            <Button
              ...
              classes={{ label: (classes as Record<string, string>).buttonLabel } as Record<string, string>}
              aria-label={t('RefreshPaymentStatus')}
            >
              <RefreshIcon className={(classes as Record<string, string>).refreshIcon} />
...
const mapStateToProps = ({ errors: { list } }: { errors: { list: unknown } }) => ({
  errorsList: list
});

const mapDispatchToProps = (dispatch: Dispatch) => ({
  importActions: {
    getPaymentInfo: bindActionCreators(getPaymentInfo as never, dispatch as never),
    getPaymentStatus: bindActionCreators(getPaymentStatus as never, dispatch as never),
    ...
    validateAppleSession: bindActionCreators(validateAppleSession as never, dispatch as never)
  }
});

const styled = withStyles(styles)(PaymentWidget as never);

const translated = translate('Elements')(styled as never);

const connected = connect(mapStateToProps, mapDispatchToProps)(translated as never) as unknown as React.ComponentType<Record<string, unknown>>;
