...
const useHeaders = (headers?: HeaderItem[][]) => {
  const classes = useStyles() as unknown as Record<string, string>;
  const [className] = useState(generateUUID());
  ...
  useEffect(() => {
    ...
    const dsgContainer =
      rootContainer.getElementsByClassName('dsg-container')[0] as HTMLElement;
    const dsgRowHeader = dsgContainer.querySelector('.dsg-row-header') as HTMLElement;
    ...
    dsgRowHeader.childNodes.forEach((node) => {
      widths.push((node as HTMLElement).offsetWidth);
    });
    ...
    headers.forEach((header) => {
      ...
      header.forEach((headerItem, headerItemIndex) => {
        ...
        headerItemContentElement.innerText = headerItem.title as string;
      ...
      dsgContainer.addEventListener('scroll', (event) => {
        dsgAdditionalHeadersSlider.style.marginLeft = `${-(event.target as HTMLElement)
          .scrollLeft}px`;
      });
    });
