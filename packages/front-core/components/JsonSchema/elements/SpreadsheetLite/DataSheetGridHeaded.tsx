...
interface DataSheetGridHeadedProps {
  headers?: unknown;
  columns?: ColumnConfig[];
  errors?: SchemaError[];
  setActiveCellRef?: (ref: Record<string, unknown>) => void;
  isMobile?: boolean;
  props: Record<string, unknown>;
  onImportCallback?: (...args: unknown[]) => void;
  [key: string]: unknown;
}
...
const DataSheetGridHeaded = ({
  ...
  onImportCallback,
  ...props
}: DataSheetGridHeadedProps) => {
  const ref = React.useRef(null);
  const [rows, setRows] = React.useState<unknown[]>([]);
  const className = useHeaders(headers as never);
  ...
  React.useEffect(() => {
    const value = props.value as unknown[] | undefined;
    if (
      ref.current &&
      setActiveCellRef &&
      isMobile &&
      (value?.length || 0) > rows.length
    ) {
      setActiveCellRef({
        root: ref.current,
      });
    ...
    <>
      ...
      <DynamicDataSheetGrid
        ...
        cellClassName={({ rowIndex, columnId }: { rowIndex: number; columnId?: string }) => {
          ...
          if ((error?.path || '').indexOf(columnId as string) !== -1) {
            if (rowIndex === 0 && defaultLayout)
              return classes.firstErrorCellDefault;
            if (rowIndex === (props.value as unknown[]).length - 1 && defaultLayout)
