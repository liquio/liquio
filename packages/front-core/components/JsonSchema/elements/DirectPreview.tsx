import React from 'react';
import mime from 'mime-types';
import { useTranslate } from 'react-translate';
import { Tooltip, CircularProgress, Typography } from '@mui/material';
import withStyles, { WithStyles } from '@mui/styles/withStyles';
import { Theme } from '@mui/material/styles';
import { ReactComponent as VisibilityIcon } from 'assets/img/visibility.svg';
import FileViewerDialogUntyped from 'components/FileViewerDialog';
import blobToBase64 from 'helpers/blobToBase64';
import evaluate from 'helpers/evaluate';
import classNames from 'classnames';

const FileViewerDialog = FileViewerDialogUntyped as unknown as React.ComponentType<Record<string, unknown>>;

const blueIcon = (
  <img
    src="data:image/svg+xml;base64,"
    alt="Переглянути виконавчий документ"
  />
);

const styles = (theme: Theme) => ({
  previewContainer: {
    display: 'flex',
    justifyContent: 'center',
    alignItems: 'center',
    cursor: 'pointer',
  },
  previewText: {
    fontSize: 14,
    fontWeight: 500,
    marginRight: theme.spacing(1),
    letterSpacing: '0.10px',
  },
  blueIcon: {
    color: '#0068FF',
  },
});

interface DirectPreviewProps extends WithStyles<typeof styles> {
  url: string;
  text?: string;
  rootDocument: { data: Record<string, unknown> };
  blueColor?: boolean;
  hiddenTooltip?: boolean;
  withPrint?: boolean;
}

const DirectPreview = ({
  url,
  text,
  rootDocument,
  blueColor,
  hiddenTooltip,
  classes,
  withPrint,
}: DirectPreviewProps) => {
  const [open, setOpen] = React.useState(false);
  const [blob, setBlob] = React.useState<unknown>(null);
  const [fileType, setFileType] = React.useState<string | false | null>(null);
  const [downloading, setDownloading] = React.useState(false);
  const t = useTranslate('FileDataTable');
  const evalUrl = evaluate(url, rootDocument.data);

  if (evalUrl instanceof Error) {
    console.log('Error: evaluate url error');
    return null;
  }

  const handleClickOpen = () => {
    setOpen(true);

    const fetchData = async () => {
      setDownloading(true);

      try {
        const response = await fetch(evalUrl as string);
        if (response.status !== 200) {
          throw new Error('Failed to fetch');
        }
        const responseToBlob = await response.blob();
        const decodedBlob = await blobToBase64(responseToBlob);
        const extension = mime.extension(responseToBlob.type);

        setFileType(extension);
        setBlob(decodedBlob);
      } catch (error) {
        console.error(error);
      } finally {
        setDownloading(false);
      }
    };

    fetchData();
  };

  const handleClose = () => setOpen(false);

  if (!url) return null;

  const fileSource = fileType === 'xlsx' ? blob : evalUrl;

  const Icon = downloading ? (
    <CircularProgress size={24} />
  ) : (
    <VisibilityIcon {...({ size: 24 } as unknown as Record<string, unknown>)} />
  );

  return (
    <>
      <Tooltip title={hiddenTooltip ? null : t('Preview')}>
        <div onClick={handleClickOpen} className={classes.previewContainer}>
          {text && (
            <Typography
              variant="h5"
              className={classNames({
                [classes.previewText]: true,
                [classes.blueIcon]: !!blueColor,
              })}
            >
              {text}
            </Typography>
          )}
          {blueColor ? blueIcon : Icon}
        </div>
      </Tooltip>

      <FileViewerDialog
        darkTheme={false}
        file={fileSource}
        fileName={url}
        open={open && !!fileType && !!blob}
        extension={fileType}
        onClose={handleClose}
        withPrint={withPrint}
      />
    </>
  );
};

export default withStyles(styles)(DirectPreview);
