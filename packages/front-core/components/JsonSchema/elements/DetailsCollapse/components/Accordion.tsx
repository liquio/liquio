...
interface AccordionCollapseProps extends WithStyles<typeof styles> {
  t: Translate;
  htmlBlock?: string;
  ...
  pure?: boolean;
  fullWidth?: boolean;
  option?: Record<string, unknown> & { id?: string | number };
}
...
const AccordionCollapse = ({
  ...
  let evalateTitle = evaluate(openText as string, option);
  ...
  return (
    <Accordion
      ...
      // `component` isn't in this MUI version's shipped `AccordionProps`, but Accordion
      // (built on Paper) does forward it to the rendered element at runtime.
      {...({ component: 'section' } as Record<string, unknown>)}
      elevation={0}
      square={true}
    >
      <AccordionSummary
        classes={{
          ...
          // The real MUI class key is `expandIconWrapper`, not `expandIcon` — this key
          // is silently ignored by MUI, so `classes.summaryExpandIcon` never applies.
          // Preserved as-is.
          ...({ expandIcon: classes.summaryExpandIcon } as Record<string, string>),
        }}
      ...
      >
        <Typography classes={{ root: classes.titleRoot }}>
          {(evalateTitle as React.ReactNode) || t('Open')}
        </Typography>
      </AccordionSummary>
