...
interface AddressValue {
  ...
  propertiesHasOptions?: unknown;
  [key: string]: unknown;
}

interface RenderAddressFormatProps {
  name: string;
  fields?: string[];
  errors?: unknown[];
  value: Record<string, AddressValue | undefined>;
...
const RenderAddressFormat = ({
  ...
  classes,
  notRequiredLabel,
}: RenderAddressFormatProps) => {
  const [actualAtu, setActualAtu] = React.useState<AddressValue>({});
  ...
  const handleChangeAtu = React.useCallback(
    async (...args: unknown[]) => {
      const action = async () => {
        const changedValue = (args.pop() as { data: AddressValue }).data;
  ...
  const handleChangeAddress = React.useCallback(
    async (...args: unknown[]) => {
      const action = async () => {
        ...
        switch (valueName) {
          case 'street':
            handleUpdateField(path, (changedValue as { data?: unknown })?.data);
            break;
          case 'isPrivateHouse':
            handleUpdateField(path, !!(changedValue as { data?: unknown[] })?.data?.length);
                ...
                building: {
                  ...
                  properties: {
                    ...
                    korp: {
                      ...
                      description: t('korp'),
                      maxLength: 20,
                      notRequiredLabel: (notRequiredLabel as { korp?: string })?.korp || undefined,
