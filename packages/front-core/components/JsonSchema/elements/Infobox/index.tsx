...
const appTheme = theme as unknown as { defaultLayout?: boolean; material?: boolean };

const hasLayoutSettings =
  typeof appTheme?.defaultLayout === 'boolean' || typeof appTheme?.material === 'boolean';
const defaultLayout = hasLayoutSettings ? appTheme?.defaultLayout === true : false;
const material = hasLayoutSettings ? appTheme?.material === true : true;
...
const styles = (theme: Theme) => ({
  ...
  materialContainer: {
    ...
    padding: '16px',
    alignItems: 'center',
    borderRadius: (theme as unknown as { materialContainer?: { borderRadius?: number } })?.materialContainer?.borderRadius || 0
  },
...
interface InfoboxProps extends WithStyles<typeof styles> {
  emoji?: string;
  title?: string;
  text?: string;
...
const Infobox = ({ emoji, title, text, collapseHeader, collapseText, rootDocument, t, classes, tooltip, iconType, hidden, stepName, parentValue }: InfoboxProps) => {
  const [expanded, setExpanded] = useState(false);
  const [isOverflowing, setIsOverflowing] = useState(false);
  const [open, setOpen] = React.useState<Element | false | null>(false);
  const [isMultiLine, setIsMultiLine] = useState(false);
  const textRef = useRef<HTMLDivElement | null>(null);
  const tooltipRef = useRef<HTMLDivElement | null>(null);

  const materialTextRef = (node: HTMLParagraphElement | null) => {
    if (node) {
  ...
  const clear = (str?: string) => str?.replace(/\s*style\s*=\s*(['"])[\s\S]*?\1/gi, '');
  const getTitle = (str: string) => {
    ...
    if (!(evaluatedTitle instanceof Error)) {
      cleanedHtml = clear(evaluatedTitle as string) || '';
  ...
  const getTooltip = (): string => {
    ...
    if (!(evaluatedTitle instanceof Error)) {
      return evaluatedTitle as string;
    }
