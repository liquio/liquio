...
import { Map as LeafletMap, GeoJSON as LeafletGeoJSON, Layer, LatLngExpression } from 'leaflet';
...
const GeojsonMap = ({
  ...
  const [popupContent, setPopupContent] = React.useState<FeatureProperties | null>(null);
  const [mapKey, setMapKey] = React.useState(0);
  const geoRefs = React.useRef<LeafletGeoJSON | null>(null);
  const mapRefs = React.useRef<LeafletMap | null>(null);
  ...
  const onEachFeature = React.useCallback(
    (feature: { properties: FeatureProperties }, layer: Layer) => {
      const showDialog = () => !readOnly && setPopupContent(feature.properties);

      const evalGoodsType = evaluate(goodsType as string, rootDocument.data);
      const booking =
      ...
      let layerStyle: Record<string, unknown> = {};
      if (layerSelected) {
  ...
  const renderFields = (popupContent: FeatureProperties | null) => {
      ...
      .map((field) => (
        <div key={field}>
          ...
          <Typography className={classes.popupValue}>
            {popupContent?.[field] as React.ReactNode}
          </Typography>
        </div>
      ));
  };

  const placeActive: { text?: string; active?: boolean } | undefined = React.useMemo(() => {
    const isActiveResult = evaluate(
      checkPlaceActive,
      popupContent,
      rootDocument.data,
    );
    return isActiveResult as { text?: string; active?: boolean } | undefined;
        ...
        >
          ...
          {geoJson ? (
            <GeoJSON
              ...
              ref={geoRefs}
              data={geoJson}
              onEachFeature={onEachFeature as (feature: unknown, layer: unknown) => void}
            />
          ) : null}
        </MapContainer>
