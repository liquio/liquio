...
const getLastNotEmptyIndex = (list: Array<Record<string, unknown>>): number => {
  for (let i = list.length - 1; i >= 0; i--) {
    if (Object.keys(list[i]).filter((key) => list[i][key]).length) {
      return i;
    }
  }
  return -1;
      ...
      Object.keys(items.properties as Record<string, JsonSchemaNode>).reduce((acc, propertyName, index) => {
        ...
        if (value instanceof Date) {
          value = humanDateFormat(
            new Date(value.getTime() + 2 * 60 * 60 * 1000),
            'DD.MM.YYYY',
          );
        }
...
export const output =
  ...
  (changes: Array<{ row: number; col: number; value: unknown }>, additions: Array<{ row: number; col: number; value: unknown }>): void => {
    const props = Object.keys(items.properties || {});
    let data: Array<Record<string, unknown>> = ([] as Array<Record<string, unknown>>)
      .concat(value ? (JSON.parse(JSON.stringify(value)) as Record<string, unknown>) : (undefined as unknown as Record<string, unknown>))
      .filter(Boolean);
    ([] as Array<{ row: number; col: number; value: unknown }>)
      ...
      .forEach(({ row, col, value: newValue }) => {
        ...
        if (useCellChangeHandler) {
          ...
          if (!newValue) {
            if (data[row] && data[row][propName]) {
              delete data[row][propName];
            }
            return;
          }
