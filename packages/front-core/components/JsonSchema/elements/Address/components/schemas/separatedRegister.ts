...
interface CalcTriggersParams {
  stepName: string;
  isArray?: boolean;
  isPopup?: boolean;
  // Passed by the one caller (`separatedRegisters.tsx`) but never read by this
  // function — a pre-existing unused argument, preserved as-is.
  hidden?: unknown;
}
...
interface DefaultSchemaParams {
  ...
  isPopup?: boolean;
  indexHidden?: unknown;
  hiddenFunction?: unknown;
  requiredFunction?: unknown;
}
...
const defaultSchema = ({
  ...
  hiddenFunction,
  requiredFunction,
}: DefaultSchemaParams): Record<string, unknown> => {
  const isArrayOrPopup = isArray || isPopup;
  let atu: Record<string, unknown> = {};
  let streetControl: Record<string, unknown> = {};
  ...
  const backgroundColor = (theme as Record<string, unknown>)?.indexBg || '#FFF4D7';
