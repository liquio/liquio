...
interface ControlKeyEntry {
  control?: string | null;
  key?: string | null;
}

export interface AttributeKeyPosition {
  key: string;
  control: string | null;
  line: number;
  start: number;
...
export const getAttributeKeyPositions = (
  controlKeyMap: ControlKeyEntry[] = [],
  jsonText: string
): AttributeKeyPosition[] => {
  const lines = jsonText.split('\n');
  const results: AttributeKeyPosition[] = [];
