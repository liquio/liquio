...
let configuredWorkerMode: string | null = null;

const configurePdfWorker = (config: { pdfWorkerCdn?: boolean; pdfWorkerLocal?: boolean }) => {
  ...
  if (useCdnWorker) {
    pdfjs.GlobalWorkerOptions.workerPort = null as never;
...
const PdfDocument = ({ file, customToolbar = null, open = true, withPrint = false, isPdfBlock }: PdfDocumentProps) => {
  ...
  const handleZoomChange = React.useCallback((event: { target: { value: unknown } }) => {
    const selectedZoomPercentage = event.target.value as number;
  ...
  const renderValue = React.useCallback(
    (value: unknown) => t('From', { page: value, total: numPages }),
    [numPages, t]
  );
  ...
  const handlePrint = () => {
    let base64Payload: string | undefined | null = null;
      ...
      >
        {numPagesToArray.map((page, index) => (
          <span id={`pdf-page-${page}-${salt}`} key={index + salt}>
            <Page {...({ pageNumber: page, width, tabIndex: '0' } as unknown as Record<string, unknown>)} />
          </span>
        ))}
      </Document>
    );
          ...
          <div className={classes.wrapper} ref={wrapperRef}>
            ...
            >
              <div className={classes.filterItem}>
                ...
                <Select
                  value={page}
                  onChange={handlePageChange as never}
                ...
                <IconButton
                  disabled={page === numPages}
                  onClick={() => {
                    setPage(numPages as number);
                    scrollToPage(numPages as number);
                  }}
              ...
              <div className={classes.filterItem}>
                ...
                <Select
                  ...
                  onChange={handleZoomChange as never}
                  className={classes.selectRoot}
                  classes={{
                    select: classes.select,
                    icon: classes.selectIcon
