...
const DirectPreview = ({ url, GridActionsCellItem }: DirectPreviewProps) => {
  const [open, setOpen] = React.useState(false);
  const [blob, setBlob] = React.useState<string | false>(false);
  const [fileType, setFileType] = React.useState<string | false>(false);
  ...
  const handleClickOpen = () => {
    ...
    const fetchData = async () => {
      ...
      const decodedBlob = (await blobToBase64(responseToBlob)) as string;
  ...
  const Icon = downloading ? <CircularProgress size={24} /> : <VisibilityIcon {...({ size: 24 } as unknown as Record<string, unknown>)} />;
