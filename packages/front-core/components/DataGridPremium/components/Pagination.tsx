...
interface MainScrollbar {
  _container: { scrollTop: number };
  updateScroll: () => void;
}
...
const Pagination = ({
    ...
    (event: { target: { value: number } }) => {
      ...
      setTimeout(() => {
        if (event.target.value < pageSize && mainScrollbar) {
          (mainScrollbar as MainScrollbar)._container.scrollTop = 0;
          (mainScrollbar as MainScrollbar).updateScroll();
        }
      }, 0);
    },
  ...
  const getIcons = React.useCallback((type: string) => {
  ...
  const renderItem = React.useCallback(
    (item: PaginationItemProps & { type: string }) => {
      const { type } = item;
    ...
    >
      <div className={classes.paginationContent}>
        ...
        <div className={classes.paginationSelect}>
          ...
          <Select
            value={pageSize}
            onChange={handlePageSizeChange as never}
            variant="outlined"
            className={classNames({
              [classes.select]: true,
      ...
      <MuiPagination
        ...
        renderItem={renderItem as (item: PaginationItemProps) => React.ReactElement}
        size={size}
...
const mapStateToProps = ({ app: { mainScrollbar } }: { app: { mainScrollbar: unknown } }) => ({
  mainScrollbar,
});
...
export default connect(
  mapStateToProps,
  mapDispatchToProps,
)(Pagination as never) as unknown as React.ComponentType<Record<string, unknown>>;
