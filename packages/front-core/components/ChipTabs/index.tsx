...
type AppTheme = Theme & {
  tabItemTextSchema?: Record<string, unknown>;
  chipColor?: string;
  iconButtonFill?: string;
  buttonBg?: string;
  tabsFlexNoWrap?: Record<string, unknown>;
};
    ...
    {(tabs || []).map(({ title, hidden, showWarning }, index) => {
      ...
      return (
        <Tab
          ...
          label={title}
          disabled={readOnly}
          icon={warning ? <ErrorIcon className={classes.warningIcon} /> : undefined}
          iconPosition="start"
          tabIndex={0}
          className={classNames(
            {
              ...
              [activeTabStyle as unknown as string]: index === activeIndex && !!activeTabStyle,
