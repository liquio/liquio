...
interface UserSettingsDialogViewProps {
  open?: boolean;
  ...
  onClickSave: () => void;
  onClose: () => void;
  settings?: { options?: Record<string, unknown> };
}
...
export const UserSettingsDialogView = ({
  ...
  return (
    ...
    >
      ...
      <DialogContent
        {...({ height: '400px' } as unknown as Record<string, unknown>)}
        ...
        {(schema && !jsonMode) ? <div className={classes.schemaFormWrapper}>
          <SchemaForm
            ...
            onChange={handleChangeAdapter(parsedValue, (newValue) => {
              ...
              const validateErrors = validateData(newValue as never, schema as never);
              setErrors(validateErrors as never);
            }, true) as never}
            handleSave={(newValue: unknown) => setValue(JSON.stringify(newValue, null, 2))}
          />
        </div> : <Editor
          height="400px"
          ...
          onChange={setValue as (value: string | undefined) => void}
