...
const TwoFactorCode = () => {
  ...
  const totp = useSelector((state) => (state as { auth?: { info?: { twoFactorType?: string } } })?.auth?.info?.twoFactorType);
  ...
  const handleGenerateTfa = React.useCallback(async () => {
    ...
    const { success, secret, uri } = await dispatch(generateTOTP() as never) as { success: boolean; secret: string; uri: string };

    setPending(false);

    if (!success) {
      dispatch(addMessage(new Message('TOTPGenerateError', 'error')) as never);
      return;
    }
  ...
  const handleEnableTfa = React.useCallback(async () => {
    ...
    const { success } = await dispatch(enableTOTP(totpSecret as string, code) as never) as { success: boolean };
    ...
    await dispatch(requestUserInfo() as never);

    dispatch(addMessage(new Message('TOTPEnabled', 'success')) as never);
  ...
  const handleDisableTfa = React.useCallback(async () => {
    ...
    const { success } = await dispatch(disableTOTP(code) as never) as { success: boolean };
  ...
  const handleChangeTfa = React.useCallback(
    async (event: { target: { checked: boolean } }) => {
      setError(false);
  ...
  const handleCopyToClipboard = React.useCallback((text: string) => {
    ...
    dispatch(addMessage(new Message('TOTPCopied', 'success')) as never);
  }, []);
        ...
        <div className={classes.mb}>
          ...
          <Typography className={classes.text}>
            ...
            <IconButton
              ...
              onClick={() => handleCopyToClipboard(totpSecret as string)}
          ...
          <div className={classes.actions}>
            <StringElement
              ...
              placeholder={t('Code')}
              onChange={setCode}
              error={error ? ({ message: t('InvalidCode') } as never) : null}
