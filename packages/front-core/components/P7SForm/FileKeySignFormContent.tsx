...
interface FileKeySignFormContentProps {
  ...
  handleKeyChange: (key: File | null) => void;
  handleChange: (name: string) => (event: { target: { value: string } }) => void;
  handleClose: () => void;
  handleSelectKey: () => void;
  tryToSubmit: (event: { key: string }) => void;
  ...
  return (
    <>
      ...
      <DialogActions className={classes.actions}>
        {onClose ? (
          <Button
            ...
            {...({ setId: (elementName: string) => setId(`cancel-${elementName}`) } as unknown as Record<string, unknown>)}
        ...
        <Button
          ...
          {...({ setId: (elementName: string) => setId(`sign-${elementName}`) } as unknown as Record<string, unknown>)}
          className={classes.button}
        >
      ...
      <Dialog
        ...
        id={setId('dialog')}
        className={(classes as Record<string, string>).dialog}
      >
        <DialogTitle
          id={setId('dialog alert-dialog-title')}
          className={(classes as Record<string, string>).dialogContentWrappers}
        >
          {t('SigningDataError')}
        </DialogTitle>
        <DialogContent className={(classes as Record<string, string>).dialogContentWrappers}>
          <DialogContentText id={setId('dialog alert-dialog-description')}>
            {renderHTML(signingError as string)}
          </DialogContentText>
        </DialogContent>
        <DialogActions className={(classes as Record<string, string>).dialogContentWrappers}>
          <Button
            color="primary"
            onClick={handleClose}
            autoFocus={true}
            id={setId('close-button')}
            {...({ setId: (elementName: string) => setId(`close-${elementName}`) } as unknown as Record<string, unknown>)}
          >
            {t('CloseDialog2')}
          </Button>
        </DialogActions>
...
const styled = withStyles(styles)(FileKeySignFormContent as never);
