...
import { TextField, Typography, Tooltip, Tabs, Tab } from '@mui/material';
...
const getBpmnAiConfig = () => {
  const config = getConfig();
  return config.bpmnAi;
};
...
interface BuilderHistoryResponse {
  input?: unknown;
  interactions?: BuilderHistoryItem[];
}

interface BpmnAiTabPanelProps {
...
const BpmnAiTabPanel = (props: BpmnAiTabPanelProps) => {
...
interface BpmnAiProps {
  fromCodeEditor?: boolean;
  externalCommand?: unknown;
  onToggleSplit?: () => void;
  open?: boolean;
  onInsertJSONCode?: (code: string) => void;
  onEditorFocus?: () => void;
}
...
const BpmnAi = ({
  ...
  const [hotActionPrompts, setHotActionPrompts] = React.useState<Record<string, unknown>[]>([]);
  const [search, setSearch] = React.useState<Record<string, string>>({ assistant: '', builder: '' });
  ...
  const [results, setResults] = React.useState<Record<string, Record<string, unknown>[] | undefined>>([] as unknown as Record<string, Record<string, unknown>[]>);
  const [greetingMessage, setGreetingMessage] = React.useState<Record<string, unknown>[]>([]);
  const [error, setError] = React.useState<Error | false | null>(null);
  const [lastInputs, setLastInputs] = React.useState<Record<string, string>>({
    assistant: '',
    builder: ''
  });
  const [builderInputFromHistory, setBuilderInputFromHistory] = React.useState<string | null>(null);
  const [showScrollToBottom, setShowScrollToBottom] = React.useState(false);
  const [revealTableInPrompt, setRevealTableInPrompt] = React.useState(false);
  const [hiddenButtonGroups, setHiddenButtonGroups] = React.useState<Record<number, boolean>>({});
  const [pressedInteractiveButtons, setPressedInteractiveButtons] = React.useState<Record<number, string>>({});
  ...
  const bpmnAiRef = React.useRef<HTMLDivElement>(null);
  const scrollableWrapperRef = React.useRef<HTMLDivElement>(null);
  const scrollToElementRef = React.useRef<HTMLDivElement>(null);
  ...
  useEffect(() => setOpen(propOpen || false), [propOpen]);

  const handleToggleBpmnAI = React.useCallback(() => {
    const bpmnAi = getBpmnAiConfig();
    if (!bpmnAi) return;
  ...
  const handleInteractiveButtonsOptions = async (buttonId: string | undefined, buttonTitle: string | undefined, responseIndex: number) => {
    if (!buttonId) return;
  ...
  const handleJSONCodeInsertion = (buttonId: string | undefined, responseIndex: number) => {
    if (!onInsertJSONCode) return;

    if (results['builder']?.length && buttonId === INTERACTIVE_BUTTONS_OPTIONS.INSERT_CODE) {
      const result = results['builder'][responseIndex] as { response?: { message?: string; result?: string } } | undefined;
  ...
  const handleSearch = async (props?: Record<string, unknown>) => {
    if (loading) return;
    ...
    const formInput = props?.formInput as { id?: string; title?: string } | undefined;
    const interactiveOptionId = formInput?.id;
    const interactiveOptionTitle = formInput?.title;
    const skipStep = props?.skipStep || false;
    const analyzeRoute = props?.anomaliesAnalyzeRoute;
    let response: Record<string, unknown> | Error | undefined;
    ...
    if (analyzeRoute) {
      response = await sendAiAnomaliesAnalyze({ input: '/analize' })(dispatch as never) as Record<string, unknown> | Error;
    } else {
      response = await getAiPrompts(requestPayload, activeAIMode)(dispatch as never) as Record<string, unknown> | Error;
    }

    if (currentTab === 'builder' && response) {
      const builderResponse = response as unknown as { step?: number; totalSteps?: number; progressInPercentage?: number; done?: boolean };
      setInteractiveStep(builderResponse?.step as number);
      setInteractiveTotalSteps(builderResponse?.totalSteps as number);
      setInteractiveProgressPercentage(builderResponse?.progressInPercentage as number);
      setInteractiveDone(builderResponse?.done as boolean);
    }
    ...
    if (formInput?.id !== INTERACTIVE_BUTTONS_OPTIONS.INSERT_CODE) {
  ...
  const MarkDownCodeBlock = ({ inline, className, children, ...props }: { inline?: boolean; className?: string; children: React.ReactNode; [key: string]: unknown }) => {
        ...
        <div className={classes.codeBlockWrapper}>
          {!copied ? (
            <Tooltip title={t('Copy')} className={classes.copyCodeButton}>
              <IconButton
                className={(classes as Record<string, string>).copyIcon}
            ...
            <Tooltip title={t('Copied')} className={classes.copyCodeButton}>
              <IconButton className={(classes as Record<string, string>).copyIcon} size="large">
    ...
    } else if ((children as unknown as string[])[0] !== '') {
      return (
  ...
  const MarkdownRenderer = ({ content, className }: { content: string | null; className?: string }) => {
  ...
  const formatResponseToMarkdownRenderer = (response: string) => {
  ...
  const getChatHistory = async (assistantSessionId: string, dispatch: unknown) => {
    setLoading(true);
    const getChatHistoryResponse = await getAiChatHistory(assistantSessionId)(dispatch as never) as ChatHistoryItem[];

    const humanType: Array<Record<string, unknown>> = getChatHistoryResponse
  ...
  const getBuilderHistory = async (builderSessionId: string, dispatch: unknown) => {
    setLoading(true);
    const getBuilderHistoryResponse = await getAiBuilderHistory(builderSessionId)(dispatch as never) as BuilderHistoryResponse;

    const historyInput = getBuilderHistoryResponse?.input;
    setBuilderInputFromHistory(historyInput as string);

    const userType: Array<Record<string, unknown>> | undefined = getBuilderHistoryResponse?.interactions
    ...
    const systemType = systemItems?.map((item, index) => {
      ...
      const systemEntry: { response: { message: unknown; options?: unknown } } = {
        response: {
  ...
  const renderGreetingMessage = (greetingData: Record<string, unknown>[], activeTab: number) => {
      ...
      return (
        <div className={classes.emptyStateWrapper}>
          {greetingData?.map((greeting) => (
            <>
              <Typography className={classes.emptyStateTitle}>{(greeting.title as string) || ''}</Typography>
              {greeting.content && (
                <MarkdownRenderer className={classes.emptyStateText} content={greeting.content as string} />
              )}
            </>
          ))}
        </div>
  ...
  const renderHotActionPrompts = (actionPrompts: Record<string, unknown>[]) => {
    if (!actionPrompts?.length) return null;

    return (
      <div className={classes.hotActionCardsWrapper}>
        {(actionPrompts as unknown as Array<{ id?: string; text?: string }>)?.map((prompt) => (
          <div
            key={prompt?.id}
            className={classNames({
            ...
            onClick={async () => {
              ...
              setSearch((prevSearch) => ({
                ...prevSearch,
                [currentTab]: prompt?.text as string
              }));
  ...
  const renderEmptyState = (activeTab: number) => {
    return (
      <>
  ...
  const renderAIResponse = (response: Record<string, unknown> | undefined, responseIndex: number) => {
    if (!response) return null;

    const { message, result, options } = response;
    const interactiveButtons = options as Array<{ id?: string; title?: string; isPrimary?: boolean }> | undefined;
    ...
    return (
      <>
        <div className={classes.answer}>
          ...
          {(interactiveButtons?.length ?? 0) > 0 && !shouldHideInteractiveButtons ? (
    ...
    const getAsyncData = async () => {
      try {
        ...
        if (localStorageAssistantSessionId !== userInfo?.userId) {
          localStorage.setItem('assistantSessionId', userInfo?.userId as string);
          setAssistantSessionId(userInfo?.userId as string);
        } else {
          setAssistantSessionId(localStorageAssistantSessionId as string);
        }
  ...
  React.useEffect(() => {
    const fetchExternalCommand = async () => {
      if (externalCommand) {
        ...
        setSearch(externalCommand as Record<string, string>);
        await handleSearch({
          input: externalCommand,
          anomaliesAnalyzeRoute: BPMN_AI_MODE_ROUTES.ANOMALIES_ANALYZER
        });

        dispatch(sendExternalCommand('') as never);
      }
    };
    ...
    return (
      <div className={classes.headerBottomRow}>
        ...
        <ColorButton
          ...
          className={classNames((classes as Record<string, string>).actionBtn, classes.cleanResults)}
        >
  ...
  return (
    <>
      {getConfig().bpmnAi && fromCodeEditor && !open && (
        <IconButton
          ...
          {initialLoading ? (
            ...
            <>
              ...
              <BpmnAiTabPanel value={tabValue} index={1} style={classes.tabPanelWrapper}>
                {results['assistant']?.map(({ keyWords, response, withError }: { keyWords?: string; response?: Record<string, unknown>; withError?: boolean }, index) => {
              ...
              <BpmnAiTabPanel value={tabValue} index={2} style={classes.tabPanelWrapper}>
                {results['builder']?.map(({ keyWords, response, withError }: { keyWords?: string; response?: Record<string, unknown>; withError?: boolean }, index) =>
                  !withError ? (
                    <>
