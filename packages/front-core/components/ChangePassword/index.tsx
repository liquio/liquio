...
interface ChangePasswordInfo {
  email?: string;
  services?: { local?: { data?: { isChangeRequired?: boolean } } };
}

interface ChangePasswordProps {
  info?: ChangePasswordInfo;
  opener?: (handleOpen: () => void) => React.ReactNode;
}

const ChangePassword = ({ info, opener }: ChangePasswordProps) => {
  ...
  const [serviceMessage, setServiceMessage] = React.useState<string | null>(null);
  ...
  const handleSave = React.useCallback(async () => {
    ...
    try {
      const result = await dispatch(changePasswordAction({ oldPassword, newPassword }) as never) as { success: boolean };
      ...
      dispatch(addMessage(new Message('ChangePasswordSuccess', 'success')) as never);

      handleClose();
    } catch (error) {
      setLoading(false);
      setServiceMessage((error as Error)?.message);
    }
  ...
  return (
    <>
      ...
      <Dialog
        ...
        maxWidth={'sm'}
        scroll={'body'}
        onClose={opener ? handleClose : undefined}
      >
        ...
        <DialogContent classes={{ root: classes.content }}>
          <DialogContentText>
            <StringElement
              ...
              value={email}
              readOnly={true}
              error={errors.includes('email') ? ({ message: t('RequiredField') } as never) : null}
              onChange={setEmail}
            />
            ...
            <StringElement
              ...
              variant={'outlined'}
              value={oldPassword}
              error={errors.includes('oldPassword') ? ({ message: t('RequiredField') } as never) : null}
              onChange={setOldPassword}
              type={showPasswordOld ? 'text' : 'password'}
              InputProps={{
                endAdornment: (
                  <InputAdornment position="end">
                    <IconButton onClick={setShowPasswordOld as never}>
            ...
            <StringElement
              ...
              error={
                errors.includes('newPassword')
                  ? ({ message: t('RequiredField') } as never)
                  : errors.includes('SamePassword')
                  ? ({ message: t('SamePassword') } as never)
                  : null
              }
            ...
            <StringElement
              ...
              error={
                errors.includes('PasswordsNotMatch')
                  ? ({ message: t('PasswordsNotMatch') } as never)
