interface AdminUnitConfig {
  defaultRoute?: string;
  roleName: string;
  navigation: {
    workflow: { MyWorkflow: boolean };
    users: {
      Users: boolean;
      Units: boolean;
      editable?: boolean;
      UserAccessJournal?: boolean;
      UserLoginJournal?: boolean;
    ...
    process: {
      Process: boolean;
      editable?: boolean;
      UserProcesses?: boolean;
    };
    healthcheck: { HealthCheckPage: boolean };
    ui: boolean;
  };
}

const adminUnits: Record<number, AdminUnitConfig> = {
  1000000: {
