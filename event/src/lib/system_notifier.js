...
class SystemNotifier {
  ...
  async sendEmails({
    ...
    if (!emailsToProceed || emailsToProceed.length === 0) {
      return;
    }
    ...
    try {
      ...
      const full_message =
        ...
        this.emailBody
          ...
          .replace('{eventTemplateId}', eventTemplateId)
          .replace('{eventTemplateName}', eventTemplateName)
          .replace('{error}', error);
