...
describe('MessageQueue', () => {
  ...
  describe('produce', () => {
    it('should send the message to the writing queue', async () => {
      ...
      messageQueue.produce('writingQueue', message);

      expect(messageQueue.channels.writing.sendToQueue).toHaveBeenCalledWith(undefined, Buffer.from(JSON.stringify(preparedMessage)), {
        persistent: true,
      });
