...
export class AuthService {
  ...
  async findByUserId(id) {
    ...
    try {
      global.log.save('id-request-find-by-user-id', id);
      ...
      global.log.save('id-request-find-by-user-id-error', { id, error: error.message });
    }
  }
  ...
  async getUser(accessToken) {
    ...
    // Check response.
    if (!response.userId) {
      global.log.save('login-error-id-response-without-user-id', response);
  ...
  async getUsersByIds(usersIds, withPrivateProps = false, briefInfo = false) {
    ...
    global.log.save('id-request-get-users-by-ids', bodyObject);
    const response = await HttpRequest.send({
    ...
    global.log.save('get-user-by-id-response', response);
  ...
  async sendPingRequest() {
    ...
    try {
      ...
      global.log.save('send-ping-request-to-liquio-id', responseData.response);
  ...
  async getLoginHistory({ offset = 0, limit = 10, filter = {} }) {
    ...
    global.log.save('get-login-history-request', { offset, limit, filter, query });
    const response = await HttpRequest.send({
    ...
    global.log.save('get-login-history-response', response);
  ...
  async getUserAdminActions({ offset = 0, limit = 10, filter = {} }) {
    ...
    global.log.save('get-user-admin-actions-request', { offset, limit, filter, query });
    const response = await HttpRequest.send({
    ...
    global.log.save('get-user-admin-actions-response', response);
  ...
  async getUserStatByDate({ date }) {
    global.log.save('get-user-stat-by-date-request', { date });
  ...
  async setUserPassword({ id, password }) {
    global.log.save('set-user-password', { id, password: '***' });
    ...
    global.log.save('set-user-password', response);
  ...
  async createLocalUser(options) {
    try {
      global.log.save('create-local-user-options', { ...options, password: '***' });
      ...
      global.log.save('create-local-user-request', {
        ...requestOptions,
      ...
      global.log.save('create-local-user-error', { error: error.message });
      throw error;
    }
  }
  ...
  async changePassword(email, oldPassword, newPassword) {
    try {
      ...
      global.log.save('change-user-password-request', { ...options, headers: { ...options.headers, Authorization: 'Basic ***' } });

      const response = await HttpRequest.send(options);
      const { success, error, message } = response || {};

      if (error || success === false) {
        global.log.save('change-user-password-error', error || message || 'Unknown error');
        return { success: false, error: error || message };
      }

      global.log.save('change-user-password-response', { success, message });
...
export class UserAdminActionEntity {
  public id: string;
  public user: {
    id: string;
    lastName: string;
