...
class CustomLogTemplateModel extends Model {
  ...
  /**
   * Get by document template ID.
   * @param {number} documentTemplateId Document template ID.
   * @returns {Promise<CustomLogTemplateEntity[]>} Custom log templates entities promise.
   */
  async getByDocumentTemplateIdWithCache(documentTemplateId) {
  ...
  /**
   * Get by operation type and document template ID.
   * @param {string} operationType Operation type.
   * @param {number} documentTemplateId Document template ID.
   * @returns {Promise<CustomLogTemplateEntity[]>} Custom log templates entities promise.
   */
  async getByOperationTypeAndDocumentTemplateIdWithCache(operationType, documentTemplateId) {
    // Get all entites with cache.
    const allEntities = await this.getAllWithCache();

    // Filter by template ID.
    const entities = allEntities.filter(
      (v) => v.operationType && v.operationType === operationType && v.documentTemplateId && v.documentTemplateId === documentTemplateId,
    );
