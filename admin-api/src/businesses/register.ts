...
export class RegisterBusiness {
  ...
  private importXlsxBatchSize: number;
  ...
  async getAllSyncedKeys() {
    const keys = await this.registerService.getAllSyncedKeys();
  ...
  async export(id, params) {
    ...
    try {
      ...
      global.log.save('register-export-error', error);
      throw error;
    }
  ...
  async import({ force, rewriteSchema, clearRecords, addData, file }, data, accessInfo) {
    try {
      ...
      global.log.save('register-import-error', error);
  ...
  async streamExport(id, { withData, file }) {
    let register;
    try {
      register = await this.registerService.export(id, { withData, file } as any);
    } catch (error) {
      global.log.save('register-export-error', error);
      throw error;
    }
  ...
  async importXlsx({ registerId, keyId, unique, clearRecords = false }, data, accessInfo) {
    try {
      ...
      global.log.save('register-import-xlsx-parsed', { time: Date.now() - time, size: data.length, sheets: xlsxParsed.length });
        ...
        for (const columns of dataFixedArray) {
          ...
          if (unique) {
            ...
            if (records && records.length > 0) {
              ...
              global.log.save('register-import-xlsx-item-updated', item);
              continue;
            }
          }
          ...
          global.log.save('register-import-xlsx-item-saving', item);
      ...
      if (clearRecords && loadedRecords.length) {
        ...
        global.log.save('register-import-xlsx-clear-records', result);
      }
    } catch (error) {
      global.log.save('register-import-xlsx-error', error);
      throw error;
    }
  ...
  async importXlsxBatch({ registerId, keyId }, batch, accessInfo) {
    ...
    global.log.save('register-import-xlsx-batch-imported', { batchSize: batch.length });
  ...
  async exportXlsx({ res, registerId, keyId }) {
    try {
      ...
      const buffer = xlsx.build([{ name: 'register', data: dataFixedArray } as any]);
      res.write(buffer);

      return res.end();
    } catch (error) {
      global.log.save('register-export-xlsx-error', error);
      throw error;
    }
  }
