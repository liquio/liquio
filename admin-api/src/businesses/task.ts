...
export class TaskBusiness {
   ...
   */
  async createOrUpdate(taskGroupEntity: TaskGroupEntity): Promise<TaskGroupEntity> {
    const transaction = await global.db.transaction();

    try {
      const documentTemplate = await global.models.documentTemplate.create(
        new DocumentTemplateEntity({ ...taskGroupEntity.documentTemplateEntity }),
        transaction,
      );
      const taskTemplate = await global.models.taskTemplate.create(new TaskTemplateEntity({ ...taskGroupEntity.taskTemplateEntity }), transaction);
  ...
  async deleteById(id) {
    try {
      const taskTemplate = await global.models.taskTemplate.findById(id);
      const documentTemplate = await global.models.documentTemplate.findById(taskTemplate.documentTemplateId);

      await global.models.taskTemplate.deleteById(id);
      await global.models.documentTemplate.deleteById(documentTemplate.id);
    } catch (error) {
