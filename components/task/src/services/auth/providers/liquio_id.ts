...
export class LiquioIdProvider extends Provider {
  private static singleton: LiquioIdProvider;
  basicAuthHeader: any;
  clientId: any;
  clientSecret: any;
  config: any;
   ...
   */
  async updateUser(userId, accessToken, options: any = {}) {
    // Define request body.
    ...
    global.log.save('user-info-updating-request', requestOptions);
  ...
  async getUserByEmail(email, withPrivateProps = false) {
    // Do request to search users.
    global.log.save('user-find-by-email-request', { email });
  ...
  async verifyPhoneAndSet(phone, code, accessToken) {
    ...
    global.log.save('user-phone-verification-request', { phone, code, accessToken });
    const response = await HttpRequest.send({
  ...
  async logoutOtherSessions(userId, accessToken, refreshToken) {
    ...
    try {
      ...
      global.log.save('logout-other-sessions-request-error', { error: error && error.message, body, userId });
    }
