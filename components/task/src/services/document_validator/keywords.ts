...
export class Keywords {
  ...
  static async checkValid(propertyData, propertySchema, pageObject, documentDataObject) {
    ...
    try {
      ...
      global.log.save('json-schema-validation-exception|check-valid', { error: error && error.message });
      return '';
    }
  }
  ...
  static async checkVerifiedUserInfo(propertyData, propertySchema, _pageObject, documentDataObject, _externalFunctions) {
    ...
    if (propertySchema?.calculateFields) {
      try {
        ...
        const wrapped: any = new Error(`Keywords.checkVerifiedUserInfo. evaluate calculateFields function error. ${error?.toString()}`);
        wrapped.cause = error;
        throw wrapped;
      }
    } else {
