...
export class RegisterService {
  ...
  async getRegisters(params) {
    ...
    global.log.save('register-request-get-registers');
  ...
  async getKeys(params) {
    ...
    global.log.save('register-request-get-keys');
  ...
  async startRollback({ keyId, timePoint }, { accessInfo = {} }) {
    ...
    if (response.error) {
      const error: any = new Error('Internal error. Start rollback error.');
  ...
  async rollbackRecord({ historyId, recordId, keyId }, { accessInfo }) {
    ...
    if (response.error) {
      const error: any = new Error('Internal error. Rollback record error.');
      error.details = response.error.details;
      throw error;
    }
