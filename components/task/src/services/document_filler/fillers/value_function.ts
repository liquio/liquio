 ...
 */
export class ValueFunctionFiller extends Filler {
  private static singleton: ValueFunctionFiller;
  constructor() {
    if (!ValueFunctionFiller.singleton) {
      // Init parent constructor.
      super();
      ValueFunctionFiller.singleton = this;
    }
    return ValueFunctionFiller.singleton;
  }
  ...
  async fill(schemaObject, objectToFill: any = {}, options: any = {}) {
    // Get options.
    const { workflowId, userInfo, mainPersonUserInfo, userUnits, userUnitsEntities, documents, allProcessDocuments, events } = options;
    ...
    await this.handleAllElements(schemaObject, objectToFill, async (item, itemSchema) => {
      ...
      if (!itemSchema || typeof itemSchema.value !== 'string' || !itemSchema.value.startsWith('(')) {
        return;
      }

      // Define current value.
      // Use only arrow function with params in brackets.
      // Samples: "({documents, events, documentData, user}) => ..." OR "(options) => ...".
      // Will not work: "function (...) {...}" OR "options => ..." ".
      const currentValueFunction = itemSchema.value;

      // Define arguments for function. documents and events we got in option argument.
      let user;

      // documentData is what we try to fill at that moment
      const documentData = jcopy(objectToFill);

      // Prepare user object to pass into function.
      try {
        user = this.prepareUserInfo(userInfo, mainPersonUserInfo, userUnits, userUnitsEntities);
      } catch (error) {
        global.log.save('value-function-filer-get-user-info-error', error, 'warn');
      }

      // Try calling value function and set value in document.
      let valueToSet = undefined;
      try {
        const documentsToPass = itemSchema.isCurrentOnly === false ? allProcessDocuments : documents;
        valueToSet = this.sandbox.evalWithArgs(currentValueFunction, [{ documents: documentsToPass, events, documentData, user }], {
          meta: { fn: 'ValueFunctionFiller.fill.value', workflowId },
        });
        if (valueToSet === null && !itemSchema.allowNull) {
          valueToSet = undefined;
        }
      } catch (error) {
        global.log.save('value-function-filer-function-calling-error', error, 'warn');
      }
  ...
  prepareUserInfo(userInfo, mainPersonUserInfo, userUnits, userUnitsEntities) {
    // Constants. "Imported" from UserInfoFiller
    const INFO_MAP = [
      // Without cases.
    ...
    return INFO_MAP.reduce((acc, { value: pathToGeting, infoObject: pathInUserInfoObject }) => {
      const propertyNameOrPath = pathToGeting.split('user.')[1];
      const propertyValueToSet = PropByPath.get(infoObject, pathInUserInfoObject);
      PropByPath.set(acc, propertyNameOrPath, propertyValueToSet);
      return acc;
    }, {});
  }
}
