...
export class TaskModel extends Model {
  private static singleton: TaskModel;
  ...
  async getAllByUserId(
    ...
    userUnitIds,
    userHeadUnitIds,
    { currentPage, perPage, filters, sort, onboardingTaskTemplateIds, fromCreatedAt, toCreatedAt }: any,
  ) {
    // Hide system tasks.
    ...
    const { data: tasks } = await RedisClient.getOrSetWithTimestamp(
      cacheKey,
  ...
  async getAllByWorkflowId(workflowId) {
    ...
    tasks = tasks.map(async (item) => {
      const task = this.prepareEntity(item);
      if (item.document) {
  ...
  async findById(id, { withWorkflow = false, withWorkflowTemplate = false } = {}) {
    const include: any[] = [{ model: global.models.document.model }, { model: global.models.taskTemplate.model }];
   ...
   * @note Similar to global.businesses.task.checkDraftExpired, but fetches task.
   * @param {string} documentId Document ID.
  ...
  async findPreviousStatusDocument(workflowId, taskTemplateId, typeService) {
    ...
    const task = await this.model.findOne({
      ...
      order: [[global.models.document.model, 'created_at', 'desc']],
    });
   ...
   */
  async getTaskAndDocumentMainInfo(workflowId, { onlyCurrent = true, maxCreatedAtTask, isDistinct = false }: any = {}) {
    const filters: any = {};
  ...
  async setStatusFinished(id, performerUsers?, performerUserNames?, name?) {
    ...
    if (name) {
      ...
      if (name.length > 255) {
        const error: any = new Error('Task name is too long.');
        error.details = { name };
  ...
  async getUnreadTasksCount(userId, userUnitIds, userHeadUnitIds, { filters, sort, onboardingTaskTemplateIds }) {
    ...
    const unreadTasks = await this.model.count({
      include: [{ model: global.models.workflow.model, required: true, attributes: [] }],
      where: {
  ...
  formOptionsToGetAllByUserId(
    ...
    sequelizeOptions.include = [
      ...
      },
      {
        ...
        include: _.has(filters, 'workflow_name') ? [{ model: global.models.workflowTemplate.model, attributes: ['name', 'data'] }] : [],
      },
  ...
  prepareOptionsForQuery(options) {
    ...
    if (options.filters.task_status) {
      if (options.filters.task_status === 'finished') {
        query.where.finished = true;
      }
      if (options.filters.task_status === 'deleted') {
        query.where.deleted = true;
      }
    }

    if (options.filters.task_template_id) {
      query.where.task_template_id = options.filters.task_template_id;
    }

    if (options.filters.performer_users) {
      query.where.performer_users = options.filters.performer_users;
    }
    ...
    if (options.filters.date && options.filters.date.from && options.filters.date.to) {
      const from = new Date(options.filters.date.from).toISOString();
      const to = new Date(options.filters.date.to).toISOString();
    ...
    if (options.sort && options.sort.by.length > 0) {
      if (options.sort.by.length > 1) {
        options.sort.by.forEach((column) => {
          if (column === 'tasks') {
            query.order.push([Sequelize.literal('tasks'), options.sort.order]);
          }
        });
      } else if (options.sort.by[0] === 'tasks') {
        query.order.push([Sequelize.literal('tasks'), options.sort.order[0]]);
      }
    }
  ...
  async setSignWithoutPerformerAvailable(id, userId, signWithoutPerformerAvailable = false) {
    // Update task.
    const { data: oldData } = await this.findById(id);
    const task = this.prepareForModel({
  ...
  async setSignWithoutPerformerAvailableByDocumentId(documentId, userId, signWithoutPerformerAvailable = false) {
    // Update task.
    const { data: oldData } = await this.findByDocumentId(documentId);
    const task = this.prepareForModel({
      data: { ...oldData, signWithoutPerformerAvailable },
      updatedBy: userId,
    });
