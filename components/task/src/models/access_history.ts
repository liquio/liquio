...
export class AccessHistoryModel extends Model {
  private static singleton: AccessHistoryModel;
   ...
   * @param {'added-to-head-unit'|'added-to-member-unit'|'deleted-from-head-unit'|'deleted-from-member-unit'|'added-to-admin'|'deleted-from-admin'} operationType Operation type.
  ...
  async update(unitId, ipn, operationType, data) {
    ...
    if (updatedAccessHistory.length === 1) {
      return this.prepareEntity(updatedAccessHistory[0]);
    }
  }
