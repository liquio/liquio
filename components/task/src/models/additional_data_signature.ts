...
export class AdditionalDataSignatureModel extends Model {
  private static singleton: AdditionalDataSignatureModel;
   ...
   * @param {string} data.data Signed data.
   * @param {string} data.signature Signature.
   * @param {string} data.certificate Certificate.
   * @param {string} data.cryptCertificate Crypt certificate.
   * @param {string} data.encryptedData Encrypted data.
   * @param {string} data.encryptedDataCertificate Encrypted data certificate.
   * @param {string} data.createdBy Created by user ID.
   * @param {object} data.meta Meta.
   * @returns {Promise<AdditionalDataSignatureEntity>} Created document signature entity promise.
   */
  async create({ documentId, data, signature, certificate, cryptCertificate, encryptedData, encryptedDataCertificate, createdBy, meta }) {
    // Prepare signature record.
    const signatureModel = this.prepareForModel({
      documentId,
      data,
      signature,
      certificate,
   ...
   */
  async saveEncryptedData(id, encryptedData, encryptedDataCertificate) {
    const dbResponse = await this.model.update(
      { encrypted_data: encryptedData, encrypted_data_certificate: encryptedDataCertificate },
      { where: { id } },
    );
