...
import { appendTraceMeta, getTraceMeta, getTraceId } from './async_local_storage';
...
export class ExternalReader {
  ...
  private static singleton: ExternalReader;
  ...
  async getData(
    ...
    try {
      ...
      if (error.code === 'ESOCKETTIMEDOUT' || error.cause?.code === 'ESOCKETTIMEDOUT') {
        global.log.save('external-reader-get-data-esockettimeout', {
          url,
      ...
      if (error.code === 'ECONNRESET' || error.cause?.code === 'ECONNRESET' || error.message === 'socket hang up') {
        global.log.save('external-reader-get-data-econnreset', {
          url,
          errorCode: error.code || error.cause?.code,
          error: error.message,
          service,
  ...
  async getDataByUser(
    ...
    try {
      ...
      global.log.save('external-reader-cannot-cut-data-to-log', { error, service, method });
      dataToLog = data;
    }
    global.log.save('external-reader-raw-data-response', {
      service,
      method,
    ...
    if (
      ...
      if (extraParams.deleteOldAttachmentsBeforeSave) {
        ...
        try {
          ...
          const wrapped: any = new Error(`ExternalReader.getDataByUser. Cannot delete old attachments. ${error.toString()}`);
          wrapped.cause = error;
          throw wrapped;
        }
        ...
        try {
          ...
          const wrapped: any = new Error(`ExternalReader.getDataByUser. Cannot get old attachments info for rewriting. ${error.toString()}`);
          wrapped.cause = error;
          throw wrapped;
        }
        ...
        try {
          ...
          global.log.save('external-reader|get-data-by-user|delete-file-from-file-storage-error', { error: error.toString() }, 'warn');
        }
      ...
      for (const attachment of data.data.attachments) {
        ...
        try {
          ...
          const wrapped: any = new Error(`ExternalReader.getDataByUser. Cannot upload attachment to file storage. ${error.toString()}`);
          wrapped.cause = error;
          throw wrapped;
        }
        ...
        try {
          ...
          const wrapped: any = new Error(`ExternalReader.getDataByUser. Cannot insert attachment to DB. ${error.toString()}`);
          wrapped.cause = error;
          throw wrapped;
        }
  ...
  appendExternalReaderErrors(externalReaderError) {
    ...
    if (!traceMeta) {
      global.log.save('external-reader-errors|trace-meta-not-found', { traceId: getTraceId(), externalReaderError });
      return;
    }
