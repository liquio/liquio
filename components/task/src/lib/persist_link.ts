...
export class PersistLink {
  private static singleton: PersistLink;

  config: any;
  generateQr: any;
  getLinkToFilestorageTimeout: any;
  sendTestPingUrl: any;
  ...
  async getQrAndLinkToDocument(documentId) {
    try {
      ...
      if (response.status === 200) {
        ...
        global.log.save('get-persist-link-to-document-response-error', error);
        throw error;
      }
    } catch (error) {
      global.log.save('get-persist-link-to-document-response-error', error);
  ...
  async getQrAndLinkToCase(caseId) {
    try {
      ...
      if (response.status === 200) {
        ...
        const error = new Error(`HTTP ${response.status}: ${response.statusText}`);
        global.log.save('get-persist-link-to-case-response-error', error);
        throw error;
      }
    } catch (error) {
      global.log.save('get-persist-link-to-case-response-error', error);
  ...
  async getQrAndLinkToProceeding(proceedingId, caseId) {
    try {
      ...
      if (response.status === 200) {
        ...
        const error = new Error(`HTTP ${response.status}: ${response.statusText}`);
        global.log.save('get-persist-link-to-permission-response-error', error);
        throw error;
      }
    } catch (error) {
      global.log.save('get-persist-link-to-permission-response-error', error);
  ...
  async getQrLinkToStaticFileInOpenStack(generatedFileName) {
    try {
      ...
      if (response.status === 200) {
        ...
        const error = new Error(`HTTP ${response.status}: ${response.statusText}`);
        global.log.save('get-persist-link-to-static-file-response-error', error);
        throw error;
      }
    } catch (error) {
      global.log.save('get-persist-link-to-static-file-response-error', error);
  ...
  async getLinkToStaticFileInFilestorage(fileId, definedHash) {
    try {
      ...
      if (response.status === 200) {
        ...
        const error = new Error(`HTTP ${response.status}: ${response.statusText}`);
        global.log.save('get-persist-link-to-static-file-response-error', { error, body: response.data });
        throw error;
      }
    } catch (error) {
      global.log.save('get-persist-link-to-static-file-response-error', { error, body: error.response?.data });
      throw error;
    }
  }
