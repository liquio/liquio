...
describe('TaskTemplate Controller', () => {
  let app: TestApp;

  const TEST_USER_ID = '61efddaa351d6219eee09048';
  ...
  describe('GET /task-templates', () => {
    it('requires auth', async () => {
      await expectAuthRequired(app, 'get', '/task-templates');
    });

    it('returns a list of task templates for an authenticated user', async () => {
  ...
  describe('GET /task-templates/:id', () => {
    it('requires auth', async () => {
      await expectAuthRequired(app, 'get', '/task-templates/:id');
    });
  });
});
