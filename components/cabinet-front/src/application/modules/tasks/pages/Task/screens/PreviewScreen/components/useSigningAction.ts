...
const bindActionCreators: any = reduxBindActionCreators;
...
const useSigningAction = (props: any): any => {
  const dispatch = useDispatch() as any;
  const fileListRef = React.useRef<any[]>([]);
  const rejectSignRef = React.useRef(false);
  const signingDialogCallbackRef = React.useRef<(() => void) | null>(null);
  ...
  const [signProgressText, setSignProgressText] = React.useState<string | null>(null);
  const [rejectSign, setRejectSign] = React.useState(false);
  const [showSuccessDialog, setShowSuccessDialog] = React.useState(false);
  const [fileList, setFileListState] = React.useState<any[]>([]);
  ...
  const updateRejectSign = React.useCallback((value: boolean) => {
    rejectSignRef.current = value;
    setRejectSign(value);
  }, []);

  const updateFileList = React.useCallback((value: any[]) => {
    fileListRef.current = value;
    setFileListState(value);
  }, []);

  const closeSigningDialog = React.useCallback((callback?: () => void) => {
  ...
  const getActiveSigners = (signatures: any[]): any[] => {
    const { task } = props;
    const { signerUsers } = task;

    const signers = signerUsers.filter(
      (userId: any) => !signatures?.find(({ createdBy }: any) => createdBy === userId)
    );

    return signers;
  };

  const getP7SSignDataWithCache = async (documentId: any, attachments: any[]): Promise<any[]> => {
    const cache = new Map<any, any>();

    const promises = attachments.map(async (attach: any) => {
      if (!cache.has(attach.id)) {
        try {
          const signData = await getTaskDocumentP7SSignData(documentId, attach.id);
          cache.set(attach.id, signData);
        } catch (e) {
          if (!errorTextArray.includes((e as Error).message)) {
            throw e;
          }
        }
      }
  ...
  const sign = async (signer: any, signInfo: any, iteration = 0): Promise<any> => {
    ...
    const signers = getActiveSigners(document?.signatures || []).filter(
      (userId: any) => userId !== authInfo?.userId
    );
  ...
  const signP7S = async (signer: any, isExternalP7sSign = false): Promise<any> => {
  ...
  const signP7SAttach = async (signer: any): Promise<any> => {
    ...
    const attachSignFunc = (attach: any, index: number) => async () => {
  ...
  const getEncodeCert = async (signer: any, index: number): Promise<any> => {
  ...
  const signAdditionalData = async (signer: any, options: any): Promise<any> => {
    const {
      t,
      task: { documentId }
    } = props;
    ...
    const preparedDataList: any[] = ([] as any[]).concat(dataToSign).filter(Boolean);

    if (!preparedDataList.length) {
      return Promise.resolve();
    }

    const p7sSignature: any[] = [];
  ...
  const encryptDocumentData = async (signer: any): Promise<any> => {
    const {
      task: { documentId }
    } = props;
  ...
  const getHideMainPdf = (value: any, taskDocumentData: any): any => {
  ...
  const getLiquioServerSign = (value: any, taskDocumentData: any, taskMeta: any, authInfo: any): boolean => {
    if (value && typeof value === 'boolean') return value;
  ...
  const taxSignEncryptSign = async (signer: any, dataToEncrypt: any, cryptCerts: any[]): Promise<any> => {
    ...
    const cryptedData = await signer.execute(
      ...
      cryptCerts.map((cert: any) => Buffer.from(cert, 'base64')),
      false,
      false,
  ...
  const onRejectSigning = (rejectData: any): void => {
  ...
  const signServer = async (signInfo: any, iteration = 0): Promise<any> => {
    const { t, setBusy, task, authInfo } = props;
  ...
  const validateKey = async (checkBeforeSign: any[], authInfo: any, data: any): Promise<void> => {
    const { task } = props;
    const { documentId } = task;
  ...
  const onSignHash = async (hashes: any[], signInfo: any): Promise<void> => {
    ...
    const repacked = await Promise.all(
      hashes.map((hash: any, index: number) => ({ ...hash, ...fileList[index] })).map(hashToInternalSign)
    ) as any[];
  ...
  const setFileName = (i: number): string | undefined => {
    const {
      t,
      template: {
  ...
  const getDataToSign = async () => {
    ...
    if (
      ...
      isMobile &&
      storedTimestamp &&
      Date.now() - Number(storedTimestamp) < 120000
    ) {
      const savedList = await dbStorage.getItem('fileList');

      updateFileList(savedList as any[]);
    ...
    if (isP7sSign) {
      if (!getHideMainPdf(hideMainPDF, task.document.data)) {
        try {
          ...
          const alreadySigned = errorTextArray.includes((e as Error).message);
    ...
    const preparedDataList: any[] = ([] as any[]).concat(additionaldataToSign).filter(Boolean);
  ...
  const toggleSigningDialog = React.useCallback((bool: boolean) => setShowSigningDialog(bool), []);

  const toggleRejectSigningDialog = React.useCallback(
    (bool: boolean) => setShowRejectSigningDialog(bool),
    []
  );

  const toggleSuccessDialog = React.useCallback((bool: boolean) => setShowSuccessDialog(bool), []);
  ...
  const alreadySigned = !!signatures.find(({ createdBy }: any) => createdBy === authInfo.userId);
  const alreadyRejected = !!signatureRejections.find(({ userId }: any) => userId === authInfo.userId);
