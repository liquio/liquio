...
export default async function removeHiddenStepsData(this: any): Promise<void> {
    ...
    if (!steps.includes(step)) {
      ...
      Object.keys(controls)
        ...
        .forEach((controlPath: any) => {
          ...
          const makeRequest = async ({ documentId, id }: any) => {
            setBusy(true);
          ...
          const controlData = objectPath.get(task.document.data, dataPath) as any[] | undefined;

          if (!controlData) return;

          controlData.forEach((data: any) => {
            if (isArray) {
              const { fieldName } = getArrayPath(controlPath);

              (data[fieldName as string] || []).forEach((file: any) => {
                if (!file) return;
