...
const userDataSchema = ({ t, fields, customValidation }: any): any => {
...
const calculatedFields = (schema: any = {}, rootDocument: any = {}, verifiedData: any = {}): any => {
  const calculatedVerifiedFields: Record<string, any> = {};
  let fields = schema.fields;
  const calculateFieldsFunc = schema?.calculateFields || null;
  if (calculateFieldsFunc) {
    let calculateFields = evaluate(calculateFieldsFunc, rootDocument.data) as any;
...
const mappedUserData = (userData: any): any =>
  removeEmptyFields({
...
const findAllControlPaths = (obj: any, controlName: string): string[] => {
  const paths: string[] = [];

  const findControl = (obj: any, currentPath = ''): void => {
...
export default async function validateUserData(this: any, validateStepId?: string): Promise<boolean> {
  ...
  let concatErrors: any[] = [];

  const checkFunction = async (selectFilesPath: string): Promise<void> => {
    ...
    const schema = objectPath.get(properties, schemaPath) as any;
