...
const LegacyFilePreview = FilePreview as React.ComponentType<any>;
...
const TaskPreview = ({
  ...
  const [showOrderAttachments, setShowOrderAttachments] = React.useState<any[] | false>(false);

  const renderAttachments = React.useMemo<any>(() => {
  ...
  const init = React.useCallback(async () => {
    ...
    if (orderAttachments && renderAttachments) {
      const changedOrderAttachments = evaluate(
        orderAttachments,
        task?.document?.data,
        attachments.filter(({ isGenerated }: any) => isGenerated || renderAttachments)
      ) as any;
    ...
    try {
      await Promise.all(
        attachments
          .filter(({ isGenerated }: any) => isGenerated || renderAttachments)
          .map((file: any) => {
            if (fileList[file.id]) {
  ...
  const handleDownloadPdf = (file: any, name: string) =>
  ...
  const getFileName = (index: number): string => template?.jsonSchema?.collapsePdfPreviewTitle[index];
      ...
      {renderAttachments && template?.jsonSchema?.isAttachmentsCollapsed ? (
        <div className={classes.wrapperAttachments} style={{ marginTop: 40, marginBottom: 16 }}>
          ...
          <FileDataTable
            data={
              showOrderAttachments
                ? showOrderAttachments.map((file: any) => ({ ...file, flex: 1 }))
                : attachments.map((file: any) => ({ ...file, flex: 1 }))
            }
...
const styled = withStyles(styles as any)(TaskPreview as any);
