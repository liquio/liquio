...
export default function triggerInitSignerList(this: any, props: any = {}): string | false {
  const {
    origin,
    task,
    stepId,
  ...
  const control = objectPath.get(jsonSchema.properties, controlPropsPath) as any;
  ...
  if (!checkSignersData(rootArray as any[])) return false;
