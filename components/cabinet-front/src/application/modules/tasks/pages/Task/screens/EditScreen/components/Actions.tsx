...
import styles from 'modules/tasks/pages/Task/screens/EditScreen/components/actionsStyles';
...
const { editScreenProgressStyle } = theme as any;
...
const Actions = (props: any) => {
  ...
  const openSidebar = useSelector((state: any) => state.app.openSidebar);
  ...
  const getButtonText = React.useCallback(
    (textTemplate: any, defaultText: string): string => {
      if (!textTemplate) {
        return defaultText;
      }
  ...
  const checkActive = React.useCallback(() => {
    ...
    if (jsonSchema.checkActive && typeof jsonSchema.checkActive === 'string') {
      ...
      if (result instanceof Error) {
        (result as Error & { commit: (context: any) => void }).commit({ type: 'checkActiveButton', jsonSchema });
        return false;
      }
  ...
  const firstStepPrevAction = () => {
    ...
    window.location.href = String(linkToRedirect);
  };
