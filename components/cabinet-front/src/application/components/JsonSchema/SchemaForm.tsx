...
const withLocalizationAliases = (localizationTexts: any[] = []): any[] => {
  ...
  const aliases = Object.entries(LOCALIZATION_KEY_ALIASES)
    .map(([aliasKey, sourceKey]) => {
      const sourceText = localizationTexts.find((item: any) => item?.key === sourceKey);
...
const localize = (value: any, localizationTexts: any[]): any =>
  resolveLocalizationText(value, {
    localizationTexts: withLocalizationAliases(localizationTexts)
  });

const localizeGetSample = (getSample: any, localizationTexts: any[]): any => {
  ...
  return getSample.replace(
    ...
    (match: string, quote: string, key: string) => {
...
const localizeSchema = (schema: any, localizationTexts: any[]): any => {
...
const SchemaForm = ({ schema, localizationTexts, ...props }: any) => {
...
const mapStateToProps = ({ app: { localizationTexts } = {} }: any) => ({
  localizationTexts
});

export default connect(mapStateToProps)(SchemaForm as any);
