...
const getFallbackCategories = () =>
  ([] as any[])
    .concat(...(getModules() as any[]).map((module) => module.navigation || []))
    .sort(prioritySort);

const resolveMenuName = (item: any, languageCode: string): string => {
...
const resolveMenuIcon = (iconValue: any): React.ReactNode => {
  ...
  for (const candidate of iconCandidates) {
    const IconComponent = (MuiIcons as any)[candidate];
...
const normalizeNavigationPath = (path: any): string => {
...
const resolveItemPath = (basePath: any, itemPath: any): string => {
...
const resolveMenuId = (item: any, localizedName: string): string => {
...
const mapNavigationTree = (items: any[], languageCode: string, parentPath = ''): any[] =>
  (items || []).map((item: any) => {
...
const Navigator = (props: any) => {
  ...
  React.useEffect(() => {
    storage.setItem('expandedCategories', expanded.join(','));
  }, [expanded]);
    ...
    return (
      <>
        <List className={classes.list} disablePadding={true}>
          {categories.filter(checkAccess).map((category: any, categoryKey: number) => {
              ...
              <li key={categoryKey}>
                {category.id ? (
                  <CategoryHeader
                    ...
                    {...category}
                  >
                    {children.map((child: any, childKey: number) => {
...
const mapStateToProps = ({ auth: { userUnits, info }, app: { navigationTree } }: any) => ({
  userUnits,
  userInfo: info,
  navigationTree
});

const translated = translate('Navigator')(Navigator as any);
const styled = withStyles(styles as any)(translated as any);
