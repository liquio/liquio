...
export const loadDocumentTemplate = (templateId: string | number) => (dispatch: Dispatch) =>
  api.get(`document-templates/${templateId}`, LOAD_DOCUMENT_TEMPLATE, dispatch).catch((error) => {
    Sentry.captureException(error);
    return error;
  });

export const loadTaskTemplates = (templateId: string | number) => (dispatch: Dispatch) =>
  api.get(`task-templates/${templateId}`, LOAD_TASK_TEMPLATES, dispatch).catch((error) => {
    Sentry.captureException(error);
    return error;
  });

export const loadDocumentTemplates = () => (dispatch: Dispatch) =>
...
export const jsonSchemaInjection = (payload: unknown) => (dispatch: Dispatch) =>
  dispatch({
