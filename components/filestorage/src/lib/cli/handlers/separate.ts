...
export class CliSeparateHandler extends CliHandler {
  fileModel: FileModel;
  ...
  async run(startDate, endDate) {
    ...
    const momentEndDate = moment(endDate || startDate, 'YYYY-MM-DD');
    ...
    const folderFileCounts: any = files.reduce((acc, file) => {
      if (!acc[file.folder]) {
        acc[file.folder] = 0;
      }
    ...
    for (const file of files) {
      ...
      try {
        ...
        await (this as any)[file instanceof FileEntity ? 'fileModel' : 'p7sSignatureModel'].update(file.id, { folder: newFolder });
