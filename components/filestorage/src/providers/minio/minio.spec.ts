...
describe('Minio Provider', () => {
  ...
  describe('getInfoToHandleFile', () => {
    ...
    it('should throw error when account password is missing', async () => {
      ...
      await expect(
        minioWithoutPassword.getInfoToHandleFile({
          filePath: 'test.txt',
  ...
  describe('downloadFile', () => {
    ...
    it('should handle different file paths correctly', async () => {
      const testCases = ['simple.txt', 'folder/subfolder/file.pdf', 'documents/report-2023.docx', 'media/images/photo.png'];
    ...
    it('should properly configure request timeout', async () => {
      ...
      const downloadFileRequestOptionsSpy = jest.spyOn(minio, 'downloadFileRequestOptions').mockResolvedValue({
      ...
      // Mock the HTTP request
      nock(mockConfig.server).get(`/${mockConfig.bucket}/${filePath}`).reply(200, 'content');
  ...
  describe('downloadFileAsBuffer', () => {
    ...
    it('should call downloadFileRequestOptions with correct parameters', async () => {
      ...
      // Mock the HTTP request
      nock(mockConfig.server).get(`/${mockConfig.bucket}/${filePath}`).reply(200, 'file content');
  ...
  describe('uploadFile', () => {
    ...
    it('should call uploadFileRequestOptions with correct parameters', async () => {
      ...
      // Mock the HTTP request
      nock(mockConfig.server).put(`/${mockConfig.bucket}/${filePath}`).reply(200, '', { etag: '"test-etag"' });
    ...
    it('should properly configure request timeout', async () => {
      ...
      const uploadFileRequestOptionsSpy = jest.spyOn(minio, 'uploadFileRequestOptions').mockResolvedValue({
        url: `${mockConfig.server}/${mockConfig.bucket}/${filePath}`,
        headers: {
          Date: new Date().toUTCString(),
          Authorization: 'AWS testuser:signature',
  ...
  describe('deleteFile', () => {
    ...
    it('should call getInfoToHandleFile with correct parameters', async () => {
      ...
      // Mock the HTTP request
      nock(mockConfig.server).delete(`/${mockConfig.bucket}/${filePath}`).reply(200, {});
    ...
    it('should properly configure request timeout', async () => {
      ...
      const sendSpy = jest.spyOn(HttpRequest, 'send').mockResolvedValue({ success: true });
  ...
  describe('getMetadata', () => {
    ...
    it('should get metadata successfully and return storage info', async () => {
      ...
      const getAuthInfoSpy = jest.spyOn(minio.auth, 'getAuthInfo').mockReturnValue({
        tenantToken: mockTenantToken,
        tenantPublicUrl: mockTenantPublicUrl,
      });

      // Mock the HTTP HEAD request
      const scope = nock(mockTenantPublicUrl).head(`/${mockConfig.container}`).matchHeader('x-auth-token', mockTenantToken).reply(200, '', {
    ...
    it('should call auth.getAuthInfo with correct method', async () => {
      ...
      // Mock the HTTP request
      nock(mockTenantPublicUrl).head(`/${mockConfig.container}`).reply(200, '', {
    ...
    it('should use default container when not specified', async () => {
      ...
      // Mock the auth.getAuthInfo method
      jest.spyOn(minio.auth, 'getAuthInfo').mockReturnValue({
        tenantToken: mockTenantToken,
        tenantPublicUrl: mockTenantPublicUrl,
      });
    ...
    it('should use custom container when specified', async () => {
      ...
      const scope = nock(mockTenantPublicUrl).head(`/${customContainer}`).matchHeader('x-auth-token', mockTenantToken).reply(200, '', {
    ...
    it('should properly calculate gigabytes from bytes', async () => {
      ...
      for (const testCase of testCases) {
        // Mock each request
        const scope = nock(mockTenantPublicUrl).head(`/${mockConfig.container}`).reply(200, '', {
    ...
    it('should configure request correctly', async () => {
      ...
      const sendDetailedSpy = (jest.spyOn(HttpRequest, 'sendDetailed') as any).mockResolvedValue({
        headers: {
          'x-container-bytes-used': '1000000000',
    ...
    it('should handle string showObjectCount parameter correctly', async () => {
      ...
      const scopeTrue = nock(mockTenantPublicUrl).head(`/${mockConfig.container}`).reply(200, '', {
        'x-container-bytes-used': '1000000000',
        'x-container-object-count': mockObjectCount,
      });
      ...
      const scopeFalse = nock(mockTenantPublicUrl).head(`/${mockConfig.container}`).reply(200, '', {
        'x-container-bytes-used': '1000000000',
        'x-container-object-count': mockObjectCount,
      });
      ...
      const scopeBoolTrue = nock(mockTenantPublicUrl).head(`/${mockConfig.container}`).reply(200, '', {
        'x-container-bytes-used': '1000000000',
        'x-container-object-count': mockObjectCount,
      });
    ...
    it('should handle network errors', async () => {
      ...
      // Mock a network error
      const scope = nock(mockTenantPublicUrl).head(`/${mockConfig.container}`).replyWithError('Network connection failed');
    ...
    it('should handle HTTP error responses gracefully', async () => {
      ...
      const scope = nock(mockTenantPublicUrl).head(`/${mockConfig.container}`).reply(404, 'Container Not Found', {});
