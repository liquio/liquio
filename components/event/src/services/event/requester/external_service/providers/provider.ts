...
export class Provider {
  ...
  async send(dataToSend: any): Promise<any> {
   ...
   */
  prepareLogType(type: string, ping = false, method = ''): string {
   ...
   */
  static getTrembitaSoapApiParams(serviceConfig: any, requestId?: any, trembitaConfig?: any): any {
    const {
      host,
      client,
      id: defaultRequestId,
   ...
   * @return {{url: string}}
   */
  static getTrembitaRestApiV1Params(serviceConfig: any): any {
   ...
   */
  static getTrembitaRestApiV2Params(serviceConfig: any): any {
    const { host, client, protocolVersion: defaultProtocolVersion } = global.config.trembita || {};
