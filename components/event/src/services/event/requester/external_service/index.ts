 ...
 */
export class ExternalServiceRequester {
  static singleton: ExternalServiceRequester;
   ...
   */
  constructor(config?: any, registerConfig?: any, pluginRegistry?: any) {
    // Define singleton.
  ...
  async send(data: any): Promise<any> {
    ...
    if (!decorator) {
      global.log.save('external-service-error|decorator-not-found', { providerName, documentTemplateId });
    ...
    try {
      ...
      global.log.save('external-service-error|transform-error', {
    ...
    if (!provider) {
      global.log.save('external-service-error|provider-not-found', { providerName });
    ...
    try {
      ...
      global.log.save('external-service-error|send-error', {
    ...
    try {
      ...
      global.log.save('external-service-error|set-external-id-error', {
        error: error && error.message,
        providerName,
        documentTemplateId,
      });
   ...
   */
  getDecorator(provider: string, documentTemplateId: any): any {
   ...
   */
  async setExternalIdIfNeedIt(sendingResult: any, documentModel: any): Promise<any> {
