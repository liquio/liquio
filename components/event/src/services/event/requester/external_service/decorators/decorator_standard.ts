 ...
 */
export class DecoratorStandard extends Decorator {
  /**
   * Transform.
  ...
  async transform(data: any): Promise<any> {
    ...
    if (sendAdditionalDataSignatures) {
      const additionalDataSignatures = await global.models.additionalDataSignature.getByDocumentId(documentId, 'asc');
      if (typeOf(additionalDataSignatureIndex) === 'string' && typeOf(additionalDataSignatureFilter) === 'string') {
        throw new Error(
   ...
   */
  async getFiles(filestorage: any, fileIds: any, p7sFileIds: any): Promise<any> {
    // Files container.
    const files = [];
   ...
   * @returns {string}
   */
  async getAdditionalSignatures(documentId: any): Promise<any> {
    return await global.models.additionalDataSignature.getByDocumentId(documentId);
  }
  ...
  async sign(data: any): Promise<any> {
    const sign = new Sign();
    const response: any = await sign.sign(data);
   ...
   * @returns {string}
   */
  async getFileBase64(filestorage: any, fileId: any): Promise<any> {
   ...
   */
  transformFunctionToAsync(functionString: string, allowedAsyncFunctions: any[] = []): string {
    // Define params.
