...
import { EventDelayer } from './delayer';
import { EventRequester } from './requester';
import { EventStopper } from './stopper';
import { EventUnit } from './unit';
import { EventUser } from './user';
import { EventWorkflow } from './workflow';
import { EventCleaner } from './cleaner';
...
/**
 * Event service.
 * @typedef {import('./stopper/stop_result').StopResult} StopResult Stop result.
 */
export class EventService {
  ...
  constructor(config) {
    ...
    if (!EventService.singleton) {
      ...
      this.eventDelayer = new (EventDelayer as any)(config.delayer);
      ...
      this.eventWorkflow = new (EventWorkflow as any)(config);
   ...
   * Notify.
   */
  async notify(method: any, ...options: any[]) {
    switch (method) {
      case 'email':
        return await (this.eventNotifier.sendEmail as any)(...options);
      case 'sms':
        return await (this.eventNotifier.sendSms as any)(...options);
      case 'digest':
        return await (this.eventNotifier.sendToDigest as any)(...options);
      case 'hideImportantMessages':
        return await (this.eventNotifier.hideImportantMessages as any)(...options);
      default:
