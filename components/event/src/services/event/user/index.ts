...
export class EventUser {
  ...
  async addMemberList({ unitId, userIdList, ...rest }: any): Promise<UserResult> {
    ...
    const result = await Promise.all(userIdList.map((userId: string) => this.addMember({ unitId, userId, ...rest })));
  ...
  async addMember({
    ...
    try {
      ...
      global.log.save('event-user-add-member-error', { error: exceptionMessage });
      throw error;
    }
    ...
    for (const baseUnitId of basedOn) {
      try {
        ...
        global.log.save('event-user-add-member-to-base-error', { error: error && error.message });
        throw error;
      }
    }

    // Log user's units.
    global.log.save('user-added-to-unit-members', { unitId, userId, workflowId, eventTemplate, userName, initUserName, initUserId });
  ...
  async addRequestedMember({ unitId, ipn, userName, unitModel }: any): Promise<UserResult> {
    // Check user already exist in needed unit as requested member.
    const isUserAlreadyInUnitAsRequestedMember = await this.checkIsUserAlreadyInUnitAsRequestedMember(unitId, ipn, unitModel);
    ...
    try {
      ...
      exceptionMessage = error && error.message;
      global.log.save('event-user-add-requested-member-error', { error: exceptionMessage });
      throw error;
    }
  ...
  async addHeadList({ unitId, userIdList, ...rest }: any): Promise<UserResult> {
    if (!Array.isArray(userIdList) || !userIdList.length) {
      return new UserResult({
    ...
    const result = await Promise.all(userIdList.map((userId: string) => this.addHead({ unitId, userId, ...rest })));
  ...
  async addHead({
    ...
    try {
      ...
      exceptionMessage = error && error.message;
      global.log.save('event-user-add-head-error', { error: exceptionMessage });
      throw error;
    }
    ...
    for (const baseUnitId of basedOn) {
      try {
        ...
        global.log.save('event-user-add-head-to-base-error', { error: error && error.message });
        throw error;
      }
    }

    // Log user's units.
    global.log.save('user-added-to-unit-heads', { unitId, userId, workflowId, eventTemplate, userName, initUserName, initUserId });
  ...
  async addMemberIpn({
    ...
    try {
      ...
      exceptionMessage = error && error.message;
      global.log.save('event-ipn-add-member-error', { error: exceptionMessage });
      throw error;
    }
    ...
    for (const baseUnitId of basedOn) {
      try {
        ...
        global.log.save('event-ipn-add-member-to-base-error', { error: error && error.message });
        throw error;
      }
    }

    // Log user's units.
    global.log.save('user-added-to-unit-member-ipns', { unitId, ipn, workflowId, eventTemplate, userName, initUserName, initUserId });
  ...
  async addMemberIpnList({
    ...
    ipnList = ipnList.map((v: any) => (typeOf(v) === 'number' ? `${v}` : v));
    ipnList = [...new Set(ipnList)];
    const currentUnitMemberIpnList = await this.getCurrentUnitIpnList(unitId, ipnList, unitModel, 'member');
    const ipnListAlreadyInUnit = ipnList.filter((v: any) => currentUnitMemberIpnList.includes(v));
    ...
    const newIpnListToAdd = ipnList.filter((v: any) => !currentUnitMemberIpnList.includes(v));
    ...
    // Log user's units.
    global.log.save('user-added-to-unit-member-ipns', {
      unitId,
  ...
  async addHeadIpnList({ ipnList, unitId, ...rest }: any): Promise<UserResult> {
    ...
    const result = await Promise.all(ipnList.map((ipn: string) => this.addHeadIpn({ ...rest, unitId, ipn })));
  ...
  async addHeadIpn({
    ...
    try {
      ...
      exceptionMessage = error && error.message;
      global.log.save('event-ipn-add-head-error', { error: exceptionMessage });
      throw error;
    }
    ...
    for (const baseUnitId of basedOn) {
      try {
        ...
        global.log.save('event-ipn-add-head-to-base-error', { error: error && error.message });
        throw error;
      }
    }

    // Log user's units.
    global.log.save('user-added-to-unit-head-ipns', { unitId, ipn, workflowId, eventTemplate, userName, initUserName, initUserId });
   ...
   */
  async isUserAlreadyInUnit(unitId: any, userId: any, unitModel: any, type: 'member' | 'head'): Promise<boolean> {
    // Check user already exist in needed unit as member.
    let unit: any;
    try {
      unit = await unitModel.findById(unitId);
    } catch (error: any) {
      global.log.save('event-user-add-member-or-head-check-existing-unit-error', {
   ...
   * @param {UnitModel} unitModel Unit model.
   */
  async checkIsUserAlreadyInUnitAsRequestedMember(unitId: any, ipn: any, unitModel: any): Promise<boolean> {
    try {
      ...
      return []
        ...
        .filter(Boolean)
        .some((requestedMember: any) => requestedMember.ipn === ipn);
    } catch (error: any) {
      global.log.save('event-user-add-member-or-head-check-existing-unit-error', {
        error: error && error.message,
        unitId,
   ...
   * @returns {Promise<boolean>} Is user already in unit indicator promise.
   */
  async isIpnAlreadyInUnit(unitId: any, ipn: any, unitModel: any, type: 'member' | 'head'): Promise<boolean> {
    // Check user already exist in needed unit as member.
    let unit: any;
    try {
      unit = await unitModel.findById(unitId);
    } catch (error: any) {
      global.log.save('event-user-add-ipn-member-or-head-check-existing-unit-error', {
        error: error && error.message,
        unitId,
        ipn,
      });
   ...
   */
  async getCurrentUnitIpnList(unitId: any, ipnList: any[], unitModel: any, type: 'member' | 'head'): Promise<any> {
    let unit: any;
    try {
      unit = await unitModel.findById(unitId);
    } catch (error: any) {
      global.log.save('event-user-get-unit-members-ipn-error', {
        error: error && error.message,
        unitId,
        ipn: ipnList.join(','),
      });
  ...
  async removeMember({
    ...
    try {
      ...
      exceptionMessage = error && error.message;
      global.log.save('event-user-remove-member-error', { error: exceptionMessage });
      throw error;
    }

    // Log user's units.
    global.log.save('user-removed-from-unit-members', { unitId, userId, workflowId, eventTemplate, userName, initUserName, initUserId });
  ...
  async removeMemberList({
    ...
    try {
      ...
      exceptionMessage = error && error.message;
      global.log.save('event-user-remove-member-list-error', { error: exceptionMessage });
      throw error;
    }

    // Log user's units.
    global.log.save('user-removed-from-unit-members-list', { unitId, userIdList, workflowId, eventTemplate, userName, initUserName, initUserId });
  ...
  async removeHead({
    ...
    try {
      ...
      exceptionMessage = error && error.message;
      global.log.save('event-user-remove-head-error', { error: exceptionMessage });
      throw error;
    }

    // Log user's units.
    global.log.save('user-removed-from-unit-heads', { unitId, userId, workflowId, eventTemplate, userName, initUserName, initUserId });
  ...
  async removeHeadList({
    ...
    try {
      ...
      exceptionMessage = error && error.message;
      global.log.save('event-user-remove-head-list-error', { error: exceptionMessage });
      throw error;
    }

    // Log user's units.
    global.log.save('user-removed-from-unit-heads-list', { unitId, userIdList, workflowId, eventTemplate, userName, initUserName, initUserId });
  ...
  async removeMemberIpn({
    ...
    try {
      ...
      exceptionMessage = error && error.message;
      global.log.save('event-user-remove-member-ipn-error', { error: exceptionMessage });
      throw error;
    }

    // Log user's units.
    global.log.save('user-ipn-removed-from-unit-members', { unitId, ipn, workflowId, eventTemplate, userName, initUserName, initUserId });
  ...
  async removeMemberIpnList({
    ...
    try {
      ...
      exceptionMessage = error && error.message;
      global.log.save('event-user-remove-member-ipn-list-error', { error: exceptionMessage });
      throw error;
    }

    // Log user's units.
    global.log.save('user-ipn-list-removed-from-unit-members', { unitId, ipnList, workflowId, eventTemplate, userName, initUserName, initUserId });
  ...
  async removeHeadIpn({
    ...
    try {
      ...
      exceptionMessage = error && error.message;
      global.log.save('event-user-remove-head-ipn-error', { error: exceptionMessage });
      throw error;
    }

    // Log user's units.
    global.log.save('user-ipn-removed-from-unit-heads', { unitId, ipn, workflowId, eventTemplate, userName, initUserName, initUserId });
  ...
  async removeHeadIpnList({
    ...
    try {
      ...
      exceptionMessage = error && error.message;
      global.log.save('event-user-remove-head-ipn-list-error', { error: exceptionMessage });
      throw error;
    }

    // Log user's units.
    global.log.save('user-ipn-list-removed-from-unit-heads', { unitId, ipnList, workflowId, eventTemplate, userName, initUserName, initUserId });
   ...
   * @returns {Promise<*>}
   */
  async searchUser(searchData: any, unitModel: any): Promise<any> {
    let exceptionMessage: string | undefined;
    let allNeededUsers: any[] = [];
    const { userIds = [], ids = [], codes = [], code, edrpou = [], search, basedOn = [] } = searchData;
    let { unitIds = [] } = searchData;
    ...
    let units: any[] = [];
    if (unitIds.length > 0) {
      unitIds = unitIds.map((unit: any) => parseInt(unit));
      try {
        const allUnits = await unitModel.getAll();
        units = allUnits.filter((v: any) => unitIds.includes(v.id));
      } catch (error: any) {
        exceptionMessage = error && error.message;
        global.log.save('event-user-search-user-unit-model-get-all-error', { error: exceptionMessage });
        throw error;
      }
    }
    ...
    if (basedOn.length > 0) {
      try {
        ...
        for (const unit of allUnits) {
          const includesBasedOn = unit.basedOn.some((el: any) => basedOn.includes(el));
    ...
    const unitsHeads = units.reduce((t: any[], v: any) => [...t, ...v.heads], []);
    const unitsMembers = units.reduce((t: any[], v: any) => [...t, ...v.members], []);
   ...
   * @param {UnitModel} unitModel Unit model.
   */
  async checkUserUnitsExclusiveRules(unitId: any, userId: any, unitModel: any, userIpn?: any): Promise<void> {
    ...
    const allUserUnitsIds = allUserUnits.map((v: any) => v.id);

    for (const userUnitId of allUserUnitsIds) {
      if (unitExclusiveRules.some((v: any) => v.includes(userUnitId) && v.includes(unitId) && userUnitId !== unitId)) {
  ...
  async removeMembersFromUnitsIpn({
    ...
    for (const unitId of unitIdList) {
      // Remove member IPN.
      let exceptionMessage: string | undefined;
      let ipnListWhichWereActuallyRemoved: any;
      let ipnListWhichWereNotInUnit: any;
      try {
        ...
        ipnListWhichWereActuallyRemoved = unitBeforeUpdate.membersIpn.filter((v: any) => !unitAfterUpdate.membersIpn.includes(v));
        ipnListWhichWereNotInUnit = ipnList.filter((v: any) => !ipnListWhichWereActuallyRemoved.includes(v));
      ...
      // Log user's units.
      global.log.save('user-ipn-list-removed-from-unit-members', {
        unitId,
  ...
  async removeMembersFromUnitsByIpn({
    ...
    for (const unitId of unitIdList) {
      // Remove member.
      let userIdListWhichWereActuallyRemoved: any;
      let userIdListWhichWereNotInUnit: any;
      ...
      for (const ipn of ipnList) {
        try {
          ...
          global.log.save('event-user-search-user-get-user-by-ipn-error', { error: error.toString() });
        }
      }
      ...
      try {
        ...
        userIdListWhichWereActuallyRemoved = unitBeforeUpdate.members.filter((v: any) => !unitAfterUpdate.members.includes(v));
        userIdListWhichWereNotInUnit = userIdList.filter((v: any) => !userIdListWhichWereActuallyRemoved.includes(v));
        ...
        global.log.save('event-user-remove-member-list-error', { error: error.toString() });
        throw error;
      }

      // Log user's units.
      global.log.save('user-removed-from-unit-members-list', {
        unitId,
