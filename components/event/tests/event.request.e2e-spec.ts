...
describe('EventBusiness - Request type events', () => {
  ...
  describe('Request type events with external service', () => {
    ...
    it('should handle API error when sending request to external service', async () => {
      ...
      app.nock('http://external-system.local').post('/').matchHeader('Authorization', 'Basic dGVzdC11c2VyOnRlc3QtcGFzcw==').reply(400, {
        error: 'Bad Request',
    ...
    it('should handle API error when retrieving non-existing task result', async () => {
      ...
      app.nock('http://external-system.local').post('/').matchHeader('Authorization', 'Basic dGVzdC11c2VyOnRlc3QtcGFzcw==').reply(404, {
        error: 'Not Found',
