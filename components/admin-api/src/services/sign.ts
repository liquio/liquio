...
export class Sign {
  ...
  async sign(data, externalSign = true, signType = '', base64 = false) {
    ...
    try {
      ...
      global.log.save('sign-service-sign-error', {
  ...
  async hash(data) {
    ...
    try {
      ...
      const formattedError = this.formatAxiosError(error);
      global.log.save('sign-service-hash-error', {
        error: formattedError.message,
        requestOptions: { ...requestOptions, data: '*****', headers: '*****' },
      });
      throw formattedError;
  ...
  async encrypt(cert, data) {
    ...
    try {
      ...
      const formattedError = this.formatAxiosError(error);
      global.log.save('sign-service-encrypt-error', {
        error: formattedError.message,
        requestOptions: { ...requestOptions, data: '*****', headers: '*****' },
      });
      throw formattedError;
  ...
  async convertInternalSignatureToExternal(internalSignature) {
    ...
    try {
      ...
      const formattedError = this.formatAxiosError(error);
      global.log.save('sign-service-convert-internal-signature-to-external-error', {
        error: formattedError.message,
        requestOptions: { ...requestOptions, data: '*****', headers: '*****' },
      });
      throw formattedError;
   ...
   */
  formatAxiosError(error: any) {
    if (error.response) {
      // Server responded with error status
      const status = error.response.status;
      const data = error.response.data;
      const message = `${status} - ${JSON.stringify(data)}`;
      const formattedError: any = new Error(message);
