...
export const requestTask = (taskId: string | number) => (dispatch: Dispatch) =>
  api
...
export const deleteTask = (taskId: string | number) => (dispatch: Dispatch) =>
  api
...
export const saveTaskData = (data: Parameters<typeof entityToBody>[0] | Array<Parameters<typeof entityToBody>[0]>) => (dispatch: Dispatch) => {
  const {
    workflow: { versions },
  } = store.getState();

  const taskList = ([] as Array<Parameters<typeof entityToBody>[0]>).concat(data).map(entityToBody);
...
export const changeTaskData = (taskId: string | number, data: unknown) => ({
  type: CHANGE_TASK_DATA,
  payload: { taskId, data },
});

export const undoTaskData = (taskId: string | number) => ({
  type: UNDO_TASK_DATA,
  payload: { taskId },
});
