...
const Header = ({
  ...
  const widgets: any[] = ([] as any[]).concat(...(modules as any[]).map((module) => module.appbar || []));

  const checkAccessWrapp = ({ access }: any) => !access || checkAccess(access, userInfo, userUnits);

  const externalCommand = useSelector((state: any) => state.bpmnAi.externalCommand);
  ...
  return (
    <header className={classes.header}>
      <Snackbars errors={errors} onClose={(errorIndex: number) => () => actions.closeError(errorIndex)} />
...
export default withStyles(styles as any)(connected as any);
