...
const getUserOptionId = (option?: UserOption | null) => option?.id || option?.userId || option?.value || '';

const getUserOptionLabel = (option: UserOption | null) => {
  if (!option) {
    return '';
  }
...
const getUnitOptionLabel = (option: UnitOption | null) => {
  if (!option) {
    return '';
  }
...
interface AccessDialogProps {
  ...
  onSave: () => void;
  onAddGrant: () => void;
  onUpdateGrant: (index: number, patch: Partial<Grant>) => void;
  searchUsers?: (params: Record<string, unknown>, query: string, options: { silent: boolean }) => Promise<UserOption[]>;
}
...
const AccessDialog = ({
  ...
  const [loadingGrantIndex, setLoadingGrantIndex] = React.useState<number | null>(null);
  ...
  const getSelectedUser = (subjectId: string) => {
    if (!subjectId) {
      return null;
    }
  ...
  const getSelectedUnit = (subjectId: string) => {
    if (!subjectId) {
      return null;
    }
  ...
  const handleSubjectTypeChange = (index: number, subjectType: string) => {
    onUpdateGrant(index, { subjectType, subjectId: '' });
  };

  const handleUserSearch = (index: number, value: string) => {
  ...
  const renderSubjectField = (grant: Grant, index: number) => {
    ...
    return (
      <Autocomplete
        ...
        onChange={(event, option) =>
          onUpdateGrant(index, { subjectId: option ? String(getUserOptionId(option)) : '' })
        }
        renderInput={(params) => (
          <TextField
            {...params}
