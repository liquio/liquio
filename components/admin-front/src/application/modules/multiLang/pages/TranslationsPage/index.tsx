...
const ColorButton = withStyles((theme: Record<string, unknown>) => ({
  root: {
    color: theme.buttonBg as string,
    background: theme.searchInputBg as string,
    borderRadius: 4,
    paddingLeft: 10,
    '&:hover': {
      background: theme.listHover as string
    },
    '& svg': {
      fill: theme.buttonBg as string
    },
    '& img': {
      fill: theme.buttonBg as string
    }
...
const TranslationsPage = ({ loading: loadingProp, location, title }: TranslationsPageProps) => {
  const t = useTranslate('TranslationsPage');
  const [open, setOpen] = React.useState(false);
  const [errors, setErrors] = React.useState<unknown[]>([]);
  const [langCodes, setLangCodes] = React.useState<LangCode[]>([]);
  const [langCode, setLangCode] = React.useState<Record<string, unknown>>({});
  const [deleting, setDeleting] = React.useState<DeletingState | false>(false);
  ...
  const [recordValue, setRecordValue] = React.useState<LocalizationRecord[]>([]);

  const languagesRef = React.useRef<HTMLInputElement>(null);
  const textsRef = React.useRef<HTMLInputElement>(null);
  ...
  const schemaRecord = React.useMemo(() => {
    ...
    langCodes.forEach((item) => {
      (schema.properties as Record<string, unknown>)[item.code] = {
        type: 'string',
        darkTheme: true,
        variant: 'outlined',
  ...
  const fetchLocalizationsData = React.useCallback(async () => {
    const codes = (await dispatch(getLanguages() as never)) as LangCode[];
    setLangCodes(codes);
    setChosenCodes(codes.map((item) => item.code));
  }, [dispatch]);

  const handleCreateLocalization = React.useCallback(async () => {
    const validateErrors = validateData(langCode, schemaLangCode as never);

    setErrors(validateErrors as unknown[]);

    if (validateErrors && (validateErrors as unknown[]).length) {
      return;
    }
  ...
  const handleDeleteLocalization = React.useCallback(
    async (item: string) => {
      try {
        setLoading(true);
        await dispatch(deleteLanguage(item) as never);
        setLoading(false);
        setDeleting(false);
        fetchLocalizationsData();
      } catch (error) {
  ...
  const handleDeleteTranslation = React.useCallback(
    async (lanCode: string, key: string) => {
      try {
        setLoading(true);
        await dispatch(deleteTranslation(lanCode, key) as never);
        setLoading(false);
        setDeleting(false);
  ...
  const handleDeleteItem = React.useCallback(() => {
    if (deleting && deleting.type === 'language') {
      handleDeleteLocalization(deleting?.value?.code as string);
    } else if (deleting && deleting.type === 'translation') {
      const keys = Object.keys(deleting.value).filter((key) => key !== 'value');

      keys.forEach((languageCode) => {
        handleDeleteTranslation(languageCode, deleting.value.value as string);
      });
    }
  }, [deleting, handleDeleteLocalization, handleDeleteTranslation]);

  const handleOpenRecord = React.useCallback(
    async (row: LocalizationRecord) => {
      setOpening(row?.value as string);
      ...
      result.forEach((item) => {
        dataToSchema[item?.localizationLanguageCode as string] = item?.value;
      });
  ...
  const handleCreateTranslation = React.useCallback(async () => {
    const validateErrors = validateData(record, schemaRecord as never);
    ...
    try {
      const saveFunctions = async (item: { localizationLanguageCode: string; key: unknown; value: unknown }) => {
        if (
          !editing ||
          ...
          await dispatch(deleteTranslation(item?.localizationLanguageCode, oldKey as string) as never);
          await dispatch(
            createTranslation({
              localizationLanguageCode: item?.localizationLanguageCode,
              key: item?.key,
          ...
          await dispatch(
            updateTranslation(item?.localizationLanguageCode, item?.key as string, {
              value: item?.value
            }) as never
          );
        }
      ...
      for (let i = 0; i < codesToSave.length; i++) {
        ...
        await saveFunctions(codesToSave[i] as never);
      }
      ...
      if (recordValue.length > codesToSave.length) {
        ...
        for (let i = 0; i < arrayForDelete.length; i++) {
          ...
          await dispatch(
            deleteTranslation(
              arrayForDelete[i].localizationLanguageCode as string,
              arrayForDelete[i].key as string
            ) as never
          );
        }
        setLoading(false);
  ...
  const handleExportLanguage = React.useCallback(async () => {
    ...
    if (blob instanceof Error) {
      dispatch(addMessage(new Message('ExportTranslationError', 'error')) as never);
      setExporting(false);
      return;
    }
    ...
    dispatch(addMessage(new Message('ExportTranslatesSuccess', 'success')) as never);
  ...
  const handleExportTexts = React.useCallback(async () => {
    ...
    const chosenTexts = tableProps.data
      .filter((item) => tableProps.rowsSelected.includes(item?.key as string))
      .flatMap((item) => {
    ...
    if (blob instanceof Error) {
      dispatch(addMessage(new Message('ImportTranslatesError', 'error')) as never);
      setExporting(false);
      return;
    }
  ...
  const handleImportLanguages = React.useCallback(
    async (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      const result: unknown = await dispatch(importLanguages(file) as never);
      ...
      dispatch(addMessage(new Message('ImportTranslatesSuccess', 'success')) as never);
    },
  ...
  const handleImportTexts = React.useCallback(
    async (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      const result: unknown = await dispatch(importTexts(file) as never);
    ...
    >
      <Content>
        <div className={classes.root}>
          <DataTable
            ...
            columns={[
              ...
              ...langCodes.map((el) => ({
                ...
                selector: (row: LocalizationRecord) => (row.translations as Record<string, unknown>)?.[el.code]
              })),
              {
                id: 'actions',
                padding: 'checkbox',
                ...
                disableClick: true,
                name: t('Actions'),
                render: (_: unknown, row: LocalizationRecord) => (
                  <div style={{ display: 'flex' }} key={row?.id as string}>
          ...
          <Dialog open={open} onClose={() => setOpen(false)} fullWidth={true} maxWidth="sm" scroll="body">
            ...
            <DialogContent>
              ...
              <SchemaForm
                ...
                onChange={handleChangeAdapter(langCode, setLangCode as never, true)}
              />
          ...
          >
            ...
            <DialogContent>
              <SchemaForm
                ...
                errors={errors}
                schema={schemaRecord}
                onChange={handleChangeAdapter(record, handleChangeRecord as never, true)}
              />
          ...
          <ConfirmDialog
            open={!!(deleting && deleting.value)}
            darkTheme={true}
            title={t('DeletePrompt')}
            description={t('DeletePromptDescription', {
              deleting: (deleting && (deleting.value?.value || deleting.value?.code)) as string
            })}
            loading={loading}
...
export default asModulePage(TranslationsPage as never);
