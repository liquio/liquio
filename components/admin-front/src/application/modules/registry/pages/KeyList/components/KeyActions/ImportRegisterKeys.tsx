...
interface ImportRegisterKeysProps {
  ColorButton: React.ComponentType<Record<string, unknown>>;
  actions: { addMessage: (message: unknown) => void };
  loading?: boolean;
}

const ImportRegisterKeys = ({ ColorButton, actions, loading: loadingOrigin }: ImportRegisterKeysProps) => {
  const [open, setOpen] = React.useState(false);
  const [loading, setLoading] = React.useState(false);
  const [importingStatus, setStatus] = React.useState<ImportStatus[]>([]);
  const [filesInfo, setFileInfo] = React.useState<FileInfo[]>([]);
  const [openImportDialog, setOpenImportDialog] = React.useState(false);
  const [, setRerender] = React.useState<unknown>(false);
  ...
  const handleOpenImportDialog = async ({ target }: React.ChangeEvent<HTMLInputElement>) => {
    setOpen(true);
    ...
    for (let i = 0; i < files.length; i++) {
      const file = await readFileAsync(files[i]) as unknown as { key: { id: string; name: string }; register: { id: string; name: string } } | Error;
  ...
  const checkProcessingStatus = (importId: string, fileInfo: FileInfo) => {
    const interval = setInterval(async () => {
      queueFactory.push(async () => {
        const processing = (await dispatch(checkImportPreparingStatus(importId) as never)) as { status: string } | Error;
        ...
        const uniqProcessingItems = importingStatus.reduce((acc: Record<string, ImportStatus>, item) => {
          const { importId } = item;
          return {
            ...acc,
            [importId]: item
  ...
  const renderFilesInfo = () => {
    const groupedFilesInfo = filesInfo.reduce((acc: Record<string, FileInfo[]>, fileInfo) => {
...
const connected = connect(null, mapDispatchToProps)(ImportRegisterKeys as never);
