...
interface ExportRegisterKeysProps {
  registerId: string;
  importActions: {
    checkExportPreparingStatus: (id: string) => Promise<{ status: string } | Error>;
    downloadPreparedRegister: (id: string) => Promise<Blob | Error>;
    startPreparingExport: (body: unknown) => Promise<{ exportId: string } | Error>;
    addMessage: (message: unknown) => void;
  };
  ColorButton: React.ComponentType<Record<string, unknown>>;
  data?: RegisterKeyItem[];
  loading?: boolean;
}
...
const ExportRegisterKeys = ({
...
}: ExportRegisterKeysProps) => {
  const t = useTranslate('RegistryListAdminPage');
  const [loading, setLoading] = React.useState(false);
  const [downloading, setDownloading] = React.useState(false);
  const [checked, setChecked] = React.useState<string[]>([]);
  const [open, setOpen] = React.useState(false);
  const [onlySchema, setOnlySchema] = React.useState<{ checked?: boolean }>({});
  ...
  const downloadRegisterKey = async (exportId: string, keyId: string) => {
  ...
  const checkProcessingStatus = (exportId: string, keyId: string) => {
  ...
  const startPreparing = async () => {
    ...
    checked.forEach((keyId) => {
      queueFactory.push(async () => {
        const body: { keyId: string; options?: { onlySchema: boolean } } = {
          keyId,
        };
  ...
  const handleOnlySchemaChange = (event: { checked?: boolean }) => {
    setOnlySchema(event);
  };
  ...
  const renderLabel = (item: RegisterKeyItem) => (
  ...
  return (
    <>
      ...
      <Dialog open={open} fullWidth={true} maxWidth="sm" onClose={handleClose}>
        ...
        <DialogContent>
          <FormGroup {...({ error: !!error } as Record<string, unknown>)}>
            {keysExist ? (
              <>
                {(data || []).map((item) => (
                  <FormControlLabel
                    ...
                    onChange={(event) => handleCheck(event as React.ChangeEvent<HTMLInputElement>, item.id)}
                    label={renderLabel(item)}
                  />
                ))}
                <div className={classes.formWrapper}>
                  <SchemaForm
                    value={onlySchema as never}
                    onChange={handleChangeAdapter(
                      onlySchema as never,
                      handleOnlySchemaChange as never,
                    )}
                    schema={{
                      type: 'object',
                      properties: {
...
export default connect(null, mapDispatchToProps)(ExportRegisterKeys as never) as unknown as React.ComponentType<
