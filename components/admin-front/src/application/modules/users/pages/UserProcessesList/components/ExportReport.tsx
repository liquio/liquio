...
interface ExportReportState {
  ...
  canceled: boolean;
  rendering: boolean;
  openEmptyFiltersDialog: boolean;
  total: number;
  process: number;
  results: LogRow[];
}
...
interface ExportReportProps {
  t: (key: string) => string;
  actions: { loadAllDataRequests: (params: { rowsPerPage: number }) => Array<() => Promise<unknown>> };
...
const ExportReport = ({
  ...
  React.useEffect(() => {
    processQueue.on('end', async () => {
      if (state.canceled) {
        ...
        const result = Handlebars.compile(htmlTemplate)({
          ...
          exporter: formatUserName(userInfo.name as string),
        });
    ...
    processQueue.on('success', (result: unknown) => {
      // `.jobs` is a real runtime property of the `queue` package (see its
      // source), just not declared in its shipped `.d.ts`.
      state.process = state.total - (processQueue as unknown as { jobs: unknown[] }).jobs.length + 1;
      state.results = ([] as LogRow[]).concat(state.results, result as LogRow);
    });
...
const authorized = withAuthorization(ExportReport as never);
export default translate('UserProcessesListPage')(authorized as never) as unknown as React.ComponentType<
