...
const SetPasswordMenuItem = ({ user, onClose }: SetPasswordMenuItemProps) => {
  ...
  const handleSavePassword = React.useCallback(async () => {
    ...
    try {
      await dispatch(
        setPasswordAction(user?.id as string, {
          password,
        }) as never,
      );
      ...
      dispatch(addMessage(new Message('handleSavePasswordSuccess', 'success')) as never);
    } catch (error) {
      setLoading(false);
