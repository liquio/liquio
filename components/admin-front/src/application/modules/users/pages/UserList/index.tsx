...
const withStylesCss = makeStyles((theme: { palette: { primary: { main: string } } }) => ({
  checkbox: {
    '& svg': {
...
interface UserListPageProps {
  ...
  title: string;
  userActions: { createUser: (data: Record<string, unknown>) => Promise<unknown>; addMessage: (message: unknown) => void };
  actions: { load: () => void; onFilterChange: (filters: unknown) => void; [key: string]: unknown };
  loading?: boolean;
  location: unknown;
  history: unknown;
  userUnits: unknown[];
...
const UserListPage = (props: UserListPageProps) => {
  const config = getConfig() as unknown as { passwordAuth?: boolean };
  ...
  const [onboardingTaskId, setOnboardingTaskId] = React.useState<string | null>(null);
  ...
  const tableProps = {
    ..._.merge(settings, dataTableAdapter(props as never, endPoint as never))
  };
...
const translated = translate('UserListPage')(UserListPage as never);
const module = asModulePage(translated as never);
const connected = connect(mapStateToProps as never, mapDispatchToProps)(module as never);
export default dataTableConnect(endPoint)(connected as never) as unknown as React.ComponentType<
