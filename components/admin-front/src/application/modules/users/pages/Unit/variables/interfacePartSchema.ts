...
interface AdditionalInterfaceProperty {
  key: string;
  text?: string;
}
...
interface AdditionalInterface {
  key: string;
  description?: string;
  properties?: AdditionalInterfaceProperty[];
}

export default (t: (key: string) => string): JsonSchemaNode => {
  const config = getConfig();
  const additionalInterfaces = (config?.additionalInterfaces as AdditionalInterface[]) || [];
  const additionalNavigation: Record<string, JsonSchemaNode> = {};
