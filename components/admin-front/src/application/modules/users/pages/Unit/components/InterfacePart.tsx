import React from 'react';
import objectPath from 'object-path';

import Preloader from 'components/Preloader';
import useTable from 'services/dataTable/useTable';
import { SchemaForm } from 'components/JsonSchema';
import propertiesEach from 'components/JsonSchema/helpers/propertiesEach';
import type { JsonSchemaNode } from 'components/JsonSchema/types';

import schema from 'modules/users/pages/Unit/variables/interfacePartSchema';

interface UiFilter {
  filter: string;
  isActive?: boolean;
}

interface InterfacePartProps {
  value: unknown;
  onChange: (...args: unknown[]) => void;
  readOnly?: boolean;
  t: (key: string) => string;
}

const InterfacePart = ({ value, onChange, readOnly, t }: InterfacePartProps) => {
  const { loading, data: uiFilters } = useTable({
    dataURL: 'ui-filters',
    sourceName: 'ui-filters',
    autoLoad: true
  }) as unknown as { loading: boolean; data: UiFilter[] | null };

  if (loading) {
    return <Preloader flex={true} />;
  }

  const partSchema: JsonSchemaNode = JSON.parse(JSON.stringify(schema(t)));

  propertiesEach(partSchema, value, (sc, data, path) => {
    if (!sc.uiFilter || !uiFilters) return;

    const uiFilter = uiFilters.find(({ filter }) => filter === sc.uiFilter);
    const controlPath = 'properties.' + (path as string).split('.').filter(Boolean).join('.properties.');

    if (!uiFilter || !uiFilter.isActive) {
      objectPath.del(partSchema as object, controlPath);
    }
  });

  return <SchemaForm value={value} readOnly={readOnly} schema={partSchema} onChange={onChange} />;
};

export default InterfacePart;
