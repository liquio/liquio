...
const RegistryMetrics = (props: RegistryMetricsProps) => {
  const { location, title, history } = props;
  const t = useTranslate('MetricsPage');
  const [result, setResult] = React.useState<RegistryMetricItem[]>([]);
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      ...
      const result = (await dispatch(getAllSynchronizationCount() as never)) as
        | RegistryMetricItem[]
        | Error;
  ...
  return (
    <LeftSidebarLayout location={location} loading={loading} title={t(title as string)} flexContent={true}>
      {error ? (
                ...
                {(filteredResult || []).map((row) => {
                  ...
                  return (
                    <TableRow
                      ...
                      onClick={() => handleRedirect(row.register_id as string)}
                      classes={{
                        root: classNames({
                          [classes.tableRowError]: !!row.errored_record_id,
                          [classes.tableRowWarning]: elastic !== (row.count as unknown as string),
...
const moduleRegistryMetrics = asModulePage(RegistryMetrics as never);
