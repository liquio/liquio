...
import CabinetMenuDialogRaw from './components/CabinetMenuDialog';
...
const CabinetMenuDialog = CabinetMenuDialogRaw as unknown as React.ComponentType<Record<string, unknown>>;

interface CabinetMenuToolbarProps {
  canEdit: boolean;
  onCreate: () => void;
      ...
      {canEdit ? (
        <>
          <input
            ...
            onChange={(event) => {
              onImportFile(event.target.files?.[0] as File);
              event.target.value = '';
            }}
            hidden={true}
          />
  ...
  return (
    <LeftSidebarLayout title={t('Title')} location={location} loading={tableProps.loading}>
      <DataTable
        ...
        getRowSortId={(row: { id: string }) => row.id}
