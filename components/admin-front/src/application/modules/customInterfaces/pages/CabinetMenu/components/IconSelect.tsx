...
export const baseIconOptions: string[] = [...muiIconNames, ...materialSymbolNames];

const isMaterialSymbolName = (value: string) => materialSymbolNames.includes(value);

const isImageSource = (value: unknown) => (
  typeof value === 'string' && (
...
const getMuiIconComponent = (iconName: string): React.ComponentType<Record<string, unknown>> | null => {
  if (!iconName) {
    return null;
  }
  ...
  return iconCandidates.reduce(
    (resolved: React.ComponentType<Record<string, unknown>> | null, candidate) => resolved || muiIcons[candidate],
    null,
  );
};

export const isSupportedIconName = (value: string): boolean => (
  isMaterialSymbolName(value) ||
  Boolean(getMuiIconComponent(value)) ||
  isImageSource(value)
);

export const renderIconPreview = (iconName: string, sx: Record<string, unknown> = {}): React.ReactNode => {
...
const getUniqueOptions = (value: string): string[] => {
...
const IconSelect = ({
  ...
  const options = React.useMemo(() => getUniqueOptions(value as string), [value]);
        ...
        >
          <Box
            sx={(theme) => ({
              p: 1,
              borderBottom: 1,
              borderColor: (theme as unknown as { borderColor: string }).borderColor,
            })}
          >
            <TextField
              fullWidth={true}
              ...
              sx={(theme) => ({
                '& .MuiInputBase-root': {
                  color: theme.palette.text.primary,
                  bgcolor: (theme as unknown as { searchInputBg: string }).searchInputBg,
                },
                        ...
                        <Box key={key} style={style} sx={{ p: 0.5 }}>
                          <ButtonBase
                            ...
                            sx={(theme) => ({
                              ...
                              '&:hover': {
                                bgcolor: selected ? theme.palette.primary.main : (theme as unknown as { listHover: string }).listHover,
                                borderColor: selected ? theme.palette.primary.main : (theme as unknown as { borderColor: string }).borderColor,
                              },
                              '&.Mui-focusVisible': {
                                outline: '2px solid',
