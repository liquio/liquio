...
import CabinetMenuActionsRaw from '../components/CabinetMenuActions';
import { getItemRoute } from '../helpers/tree';
import type { CabinetMenuItem } from '../helpers/actions';

const CabinetMenuActions = CabinetMenuActionsRaw as unknown as React.ComponentType<Record<string, unknown>>;
...
const renderResolvedIcon = (iconName: string): React.ReactNode => {
  ...
  const IconComponent = iconCandidates.reduce(
    (resolved: React.ComponentType<Record<string, unknown>> | null, candidate) =>
      resolved || (MuiIcons as unknown as Record<string, React.ComponentType<Record<string, unknown>>>)[candidate],
    null,
  );
...
const getLocalizedName = (item: TreeRow, languageCode: string | null): string => {
...
interface UseCabinetMenuColumnsParams {
  t: (key: string, params?: Record<string, unknown>) => string;
  expanded: Record<string, boolean>;
  toggleExpanded: (id: string) => void;
  localItems: CabinetMenuItem[];
  canEdit: boolean;
  handleItemAction: (action: { type: string; item?: CabinetMenuItem }) => void;
}
...
const useCabinetMenuColumns = ({
  ...
  return React.useMemo(() => [
          ...
          {row.hasChildren ? (
            <IconButton
              ...
              onClick={(event) => {
                ...
                toggleExpanded(row.id as string);
              }}
            >
              {expanded[row.id as string] === false ? <ChevronRightIcon fontSize="small" /> : <ExpandMoreIcon fontSize="small" />}
            </IconButton>
          ) : (
            <Box sx={{ width: 36 }} />
          )}
    ...
    },
    {
      ...
      name: t('Icon'),
      disableTooltip: true,
      render: (_: unknown, row: TreeRow) => renderResolvedIcon(row.icon as string),
    },
    {
      id: 'route',
      name: t('Route'),
