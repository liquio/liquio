...
interface QrTemplatesListProps {
  title?: string;
  location: unknown;
  actions: {
    requestQrTemplates: () => Promise<QrTemplate[] | Error>;
    requestQrTemplate: (id: string | number) => Promise<QrTemplate>;
    createQrTemplate: (data: QrTemplate) => Promise<QrTemplate | Error>;
    updateQrTemplate: (id: string | number, data: QrTemplate) => Promise<QrTemplate | Error>;
    deleteQrTemplate: (id: string | number, data: QrTemplate) => Promise<unknown>;
...
const QrTemplatesList = ({
  ...
  userInfo,
  userUnits,
}: QrTemplatesListProps) => {
  const t = useTranslate('QrTemplatesList');
  const classes = useStyles();

  const [loading, setLoading] = React.useState(false);

  const [list, setList] = React.useState<QrTemplate[]>([]);
  const [isRequired, setIsRequired] = React.useState(false);
  const [mode, setMode] = React.useState('');
  const [open, setOpen] = React.useState(false);
  const [template, setTemplate] = React.useState<QrTemplate>(defaultTemplate);
  const [openEditor, setOpenEditor] = React.useState(false);
  const [openConfirm, setOpenConfirm] = React.useState(false);
  const [params, setParams] = React.useState('');
  const [chosenId, setChosenId] = React.useState<string | number | null>(null);
  const [deletingItem, setDeletingItem] = React.useState<QrTemplate | null>(null);
  ...
  const handleClickEdit = async (row: QrTemplate) => {
    setChosenId(row.id as string | number);
    const result = await actions.requestQrTemplate(row.id as string | number);
    setTemplate({
      ...result,
  ...
  const handleDeleteOpen = (row: QrTemplate) => {
    setOpenConfirm(true);
    setDeletingItem(row);
  };

  const handleDelete = async () => {
    await actions.deleteQrTemplate((deletingItem as QrTemplate).id as string | number, template);

    const newList = await actions.requestQrTemplates();

    setList(newList as QrTemplate[]);
  };
  ...
  const handleSave = async () => {
    let result: QrTemplate | Error | null = null;

    if (!chosenId) {
      result = await actions.createQrTemplate({
        ...template,
        jsonMap: JSON.parse(template.jsonMap as string),
      });
    } else {
    ...
    setChosenId(result.id as string | number);
  ...
  if (!readOnly) {
    columns.push({
      ...
      width: 40,
      name: t('Actions'),
      render: (edit: unknown, row: QrTemplate) => (
  ...
  return (
    ...
    >
      ...
      <EditorDialog
        ...
        value={template[params] as string}
        handleSave={handleSave}
        onChange={(value: unknown) => setTemplate({ ...template, [params]: value })}
...
const moduled = asModulePage(QrTemplatesList as never);
