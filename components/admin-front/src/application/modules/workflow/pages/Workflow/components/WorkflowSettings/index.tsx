...
import { SchemaForm as SchemaFormRaw, handleChangeAdapter } from 'components/JsonSchema';
import StringElementRaw from 'components/JsonSchema/elements/StringElement';

import WorkflowCategorySelectRaw from '../WorkflowCategorySelect';
import styles from './styles';

const blockTaskFunc = '() => false;';

interface StatusValueParams {
  type?: string;
  label?: string;
  description?: string;
}
...
const defaultStatusesValue = ({
  ...
  label,
  description,
}: StatusValueParams) => `(documents, events) => ([
  {
...
interface WorkflowLike {
  ...
  data: {
    ...
    timeline?: { steps?: unknown[] } | Record<string, never>;
    disabledText?: string;
    isOnlyOneDraftAllowed?: boolean;
    oneDraftAllowedMessage?: string;
    [key: string]: unknown;
  };
  [key: string]: unknown;
}
...
interface Status {
  ...
  description?: string;
  calculate?: string;
  isStatusesTab?: boolean;
  taskOrEventTemplateId?: string;
}

const getCustomBody = (workflow: WorkflowLike) => ({
...
interface WorkflowSettingsProps {
  ...
  actions: { exportWorkflow: (workflowId: string | number) => Promise<Blob> };
  workflow: WorkflowLike;
  handleChangeWorkflow: (workflow: WorkflowLike) => void;
  taskTemplates: Option[];
  events: Option[];
  workflowStatuses: Option[];
  numberTemplates: Option[];
  workflowSchema: Record<string, unknown>;
  errors?: unknown[];
  setErrors: (errors: unknown[]) => void;
  unitSettingsSchema: Record<string, unknown>;
  modeler?: BpmnJsInstance & { saveSVG?: (options: { format?: boolean }, callback: (error: Error | null, svg?: string) => void) => void };
  handleSave: () => void;
  isPristine?: boolean;
}
...
const WorkflowSettings = ({
  ...
  // Deferred into the function body (not module scope) as a precaution:
  // these all sit on the `components/JsonSchema`/`components/Editor` import
  // chain that this session has repeatedly found circularly self-referencing
  // elsewhere — see TYPESCRIPT.md's DataTable/CodeEditDialog batch notes.
  ...
  const WorkflowCategorySelect = WorkflowCategorySelectRaw as unknown as React.ComponentType<Record<string, unknown>>;
  ...
  const [editorErrors, onValidate] = React.useState<unknown[]>([]);
  const [statusErrors, setStatusErrors] = React.useState<{ label?: number }>({});
  ...
  const handleChangeBlockStart = ({ block, disabledText }: { block: boolean; disabledText?: string }) => {
    const workflowUpdated = { ...workflow };
    const customFieldsUpdated = { ...customFields, block };
    const currentValue = workflowUpdated.data.entryTaskTemplateIds[0].id as string;
  ...
  }: { isOnlyOneDraftAllowed: boolean; oneDraftAllowedMessage?: string }) => {
    const workflowUpdated = { ...workflow };
  ...
  const handleChangeHidden = ({ hidden }: { hidden: boolean }) => {
    const workflowUpdated = { ...workflow };
  ...
  const handleChangeUserAccess = ({ userAccess }: { userAccess: string }) => {
    const workflowUpdated = { ...workflow };
  ...
  const handleChangeStatus = (statuses: Status[]) => {
    const mappedStatuses = statuses.map(
      ({ taskOrEventTemplateId, ...rest }) => {
        ...
        const parsed = JSON.parse(taskOrEventTemplateId) as { type?: string; sourceId?: string | number };
  ...
  const handleExportSchema = async () => {
    try {
      switch (exportType) {
        case 'xml': {
          const xmlBlob = new Blob([workflow?.xmlBpmnSchema as string], {
        ...
        case 'svg': {
          modeler?.saveSVG?.({ format: true }, function (error, svg) {
            ...
            const svgBlob = new Blob([svg as string], {
              type: 'image/svg+xml',
            });
        ...
        case 'png': {
          modeler?.saveSVG?.({ format: true }, function (error, svg) {
            ...
            const width = (container.querySelector('svg') as SVGElement).getAttribute('width') as string;
  ...
  const handleChangeStatusEditor =
    ({ index }: { index: number }) =>
      (calculate: string) => {
        ...
        workflowUpdated.data.statuses[index] = updatedStatusInfo as unknown as Status;

        handleChangeWorkflow(workflowUpdated);
      };

  const getStatusEditorValue = (status: Status) => {
  ...
  const handleChangeStatusField =
    ({ name, index }: { name: string; index: number }) =>
      (value: unknown) => {
        ...
        (workflowUpdated.data.statuses[index] as unknown as Record<string, unknown>)[name] = value;
  ...
  const changeStatusPlace = (index: number, position: 'up' | 'down') => {
    const workflowUpdated = { ...workflow };
    ...
    <>
      <SchemaForm
        ...
        onChange={handleChangeAdapter(
          workflow,
          handleChangeWorkflow as never,
          true,
          workflowSchema as never,
        )}
        schema={{
          type: 'object',
          properties: {
      ...
      <SchemaForm
        errors={errors}
        value={customFields}
        onChange={handleChangeAdapter(customFields, handleChangeBlockStart as never, true)}
        schema={{
          type: 'object',
          properties: {
            block: {
      ...
      <SchemaForm
        errors={errors}
        value={customFields}
        onChange={handleChangeAdapter({}, handleChangeHidden as never, true)}
        schema={{
          type: 'object',
          properties: {
            hidden: {
      ...
      <SchemaForm
        ...
        onChange={handleChangeAdapter(
          {},
          handleChangeUserAccess as never,
          true,
          workflowSchema as never,
        )}
                  ...
                  {expendMode ? (
                    <>
                      ...
                      <Editor
                        ...
                        onValidate={(validateErrors: { severity: number }[]) => {
                          onValidate(
      ...
      <SchemaForm
        ...
        onChange={handleChangeAdapter(
          customFields,
          handleChangeIsOnlyOneDraftAllowed as never,
          true,
        )}
        handleSave={handleSave}
        schema={{
      ...
      <div className={classes.exportBlockWrapper}>
        <SchemaForm
          value={{ exportType }}
          onChange={(name: string, value: string) => handleExportChangeType(value)}
          schema={{
            type: 'object',
            properties: {
              exportType: {
...
const styled = withStyles(styles)(WorkflowSettings as never);
