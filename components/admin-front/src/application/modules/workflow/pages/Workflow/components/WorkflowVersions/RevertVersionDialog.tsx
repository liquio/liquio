...
const RevertVersionDialog = ({
  ...
  const {
    ...
    progressText,
    onSelectChanges,
  } = useRevert(currentData as never, revertData as never, { t, workflowId, onRevert });
  ...
  return (
    ...
    >
      ...
      {busy ? (
        <DialogContent>
          {progressText ? (
            <Typography className={(classes as { progressText?: string }).progressText}>
              {progress ? (
                <CircularProgress size={12} className={(classes as { progress?: string }).progress} />
              ) : null}
              {progressText}
            </Typography>
          ) : null}
        ...
        <RevertChangesTable
          disabled={busy}
          changes={changes}
          onSelect={onSelectChanges as never}
        />
      )}
      <DialogActions>
