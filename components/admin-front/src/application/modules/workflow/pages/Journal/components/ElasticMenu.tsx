...
interface LogRecord {
  details?: { data?: { traceId?: string; logId?: string; queueMessage?: { userId?: string } } };
}

interface ElasticMenuProps {
  log: LogRecord;
}

const ElasticMenu = ({ log }: ElasticMenuProps) => {
  const config = getConfig() as unknown as { elastic?: { url?: string; patternId?: string } };

  const templateList = [
    {
      name: 'ByTraceId',
      getUrl: (log: LogRecord) => {
  ...
  const [anchorEl, setAnchorEl] = React.useState<HTMLElement | false | undefined>(false);
