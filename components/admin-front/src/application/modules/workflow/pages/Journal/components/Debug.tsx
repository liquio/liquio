...
interface RenderEditorParams {
  ...
  onValidate?: ((errors: unknown[]) => void) | null;
  readOnly?: boolean;
}

const renderEditor = ({ value, onChange, onValidate, readOnly }: RenderEditorParams) => {
...
interface DebugDetails {
  ...
  gatewayTemplateId?: string | number;
  data?: { queueMessage?: { eventTemplateId?: string | number; gatewayTemplateId?: string | number } };
}
...
interface DebugProps {
  ...
  actions: {
    ...
    requestEvent: (id: string | number) => Promise<{ jsonSchema: unknown; [key: string]: unknown }>;
    saveEventData: (body: unknown) => Promise<unknown>;
    requestGateway: (id: string | number) => Promise<{ jsonSchema: unknown; [key: string]: unknown }>;
    saveGatewayData: (body: unknown) => Promise<unknown>;
    restartProcess: (processId: string | number, body: unknown) => Promise<unknown>;
...
const Debug = (props: DebugProps) => {
  ...
  const [origin, setOriginValue] = React.useState<Record<string, unknown>>({});
  const [debugResult, setDebugResult] = React.useState('');

  if (!types.includes(type) && !types.includes(details?.serviceName as string))
    return null;
  ...
  const opedDialog = async () => {
    ...
    const result = eventTemplateId
      ...
      : await actions.requestGateway(gatewayTemplateId as string | number);
  ...
  const restartProcess = async () => {
    const messages = logs
      ...
      .map(({ details: { data } }) => data as { eventTemplateId?: string | number; gatewayTemplateId?: string | number });
    let message: Record<string, unknown> | unknown[] = {};
    ...
    const result = await actions.restartProcess(processId, {
      debug: true,
      ...((message as unknown[])[0] as Record<string, unknown>),
    });

    return result;
  };

  const lastEditString = (lastWorkflowHistory: { meta: Record<string, unknown>; updatedAt: string }) => {
    const getName = (src: Record<string, unknown>) => {
      const { name } = src;
  ...
  const checkValid = (error: unknown[]) => setValidState(!error.length);

  const savingErrorMessage = (result: { message?: string; response?: { details?: { pop: () => { lastWorkflowHistory?: unknown } }[] } }) => {
    if (result.message === 'Header Last-Workflow-History-Id expired.') {
      const error = new Error('WorkflowOldVersionError') as Error & { details?: string };

      const { details } = result.response as { details: { pop: () => { lastWorkflowHistory?: { meta: Record<string, unknown>; updatedAt: string } } }[] };

      if (details) {
        const { lastWorkflowHistory } = details.pop() as { lastWorkflowHistory?: { meta: Record<string, unknown>; updatedAt: string } };
  ...
  const handleChange = async () => {
    ...
    if (savingResult instanceof Error) {
      savingErrorMessage(savingResult as never);
      setLoading(false);
      return;
    }
...
const translated = translate('ProcessesListPage')(Debug as never);
