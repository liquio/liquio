...
interface CreateTagProps {
  ...
  selectedRow: WorkflowTag | null;
  setSelectedRow: (row: WorkflowTag | null) => void;
  actions: {
    createTag: (data: { name: string; color: string }) => Promise<unknown>;
    updateTag: (id: string | number, data: { name: string; color: string }) => Promise<unknown>;
    deleteTag: (id: string | number) => Promise<unknown>;
    getTagList: (params: string) => Promise<WorkflowTag[]>;
  };
  setWorkflowTags: (tags: WorkflowTag[]) => void;
}
...
class CreateTag extends React.Component<CreateTagProps, CreateTagState> {
  ...
  save = async () => {
    ...
    if (isEdit) {
      await actions.updateTag(selectedRow.id as string | number, {
        name: tagName,
        color: selectedColor,
      });
    } else {
  ...
  delete = async () => {
    const { selectedRow, setSelectedRow, actions, setWorkflowTags } =
      this.props;
    await actions.deleteTag((selectedRow as WorkflowTag).id as string | number);
    const result = await actions.getTagList('');
    setWorkflowTags(result);
    setSelectedRow(null);
  };
      ...
      <>
        <div className={classes.buttonsWrapper}>
          <ColorButton
            ...
            onClick={() => setSelectedRow({} as WorkflowTag)}
          >
            <AddIcon />
            {t('Create')}
          </ColorButton>
...
const styled = withStyles(styles)(CreateTag as never);
const translated = translate('TagsListPage')(styled as never);
