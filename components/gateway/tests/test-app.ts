...
import { GatewayBusiness } from '../src/businesses/gateway';
...
export class TestApp {
  ...
  static async beforeAll() {
    ...
    // Redis isn't exercised by the controllers/business under test.
  ...
  // Gateway doesn't own its own migrations/fixtures: it shares the "bpmn" schema
  // that manager's migrations create, so we point sequelize at manager's directory.
  static async applyMigrations(dbConfig) {
    ...
    // Load fixture data shared with manager's and admin-api's e2e tests.
  ...
  async init() {
    ...
    // Init gateway business.
    this.gatewayBusiness = new GatewayBusiness();
    await this.gatewayBusiness.init();

    // Init message queue, mirroring src/index.ts's boot sequence.
    this.messageQueue = new MessageQueue(config.message_queue, {
      onInit: () => {
        this.messageQueue.subscribeToConsuming(this.gatewayBusiness.createFromMessage.bind(this.gatewayBusiness));
      },
    });
  ...
  // Publish a message directly to the reading queue, as the manager service would.
