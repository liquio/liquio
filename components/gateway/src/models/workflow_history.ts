...
export class WorkflowHistoryModel extends Model {
   ...
   */
  async findLastVersionByWorkflowTemplateId(workflowTemplateId: number): Promise<WorkflowHistoryEntity | undefined> {
   ...
   */
  prepareEntity(item: any): WorkflowHistoryEntity {
    let data;
    try {
   ...
   * @returns {object}
   */
  prepareForModel(item: WorkflowHistoryEntity): any {
    return {
