...
export class Log {
   ...
   */
  addProvider(logProvider: LogProvider) {
   ...
   */
  async save(type: string, data: any = true, level: string = this.Levels.INFO_LEVEL) {
    // Define params.
   ...
   */
  async logRouter(req: any, res: any, next: any) {
    // Define params.
   ...
   */
  attachResponseLogger(req: any, res: any) {
    const time = Date.now();
