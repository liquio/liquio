...
export class GatewayModel extends Model {
   ...
   */
  async create({ gatewayTemplateId, gatewayTypeId, workflowId, name, createdBy, updatedBy, data, version }: any): Promise<GatewayEntity> {
   ...
   * @returns {Promise<GatewayEntity>} Created gateway entity promise.
   */
  async getLatestGatewayInWorkflow(gatewayTemplateId: number, workflowId: string): Promise<GatewayEntity> {
   ...
   */
  prepareEntity(item: any): GatewayEntity {
