...
export class WorkflowTemplateModel extends Model {
   ...
   */
  async findById(id: number): Promise<WorkflowTemplateEntity> {
   ...
   */
  async findIdAndNameAndErrorsSubscribersById(id: number): Promise<WorkflowTemplateEntity> {
   ...
   */
  prepareEntity(item: any): WorkflowTemplateEntity {
   ...
   * @returns {object}
   */
  prepareForModel(item: WorkflowTemplateEntity): any {
    return {
      name: item.name,
      description: item.description,
