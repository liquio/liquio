...
const CreateGroup = ({
  ...
  const [snippetGroup, setSnippetGroup] = React.useState(
    setDefaultGroup(activeSnippet),
  );
  const [snippetCode, setSnippetCode] = React.useState(
    getSavedCode(activeSnippet, 'code'),
  );
  const [triggersCode, setTriggersCode] = React.useState(
    getSavedCode(activeSnippet, 'json'),
  );
  const [additionCode, setAdditionCode] = React.useState(
    getSavedCode(activeSnippet, 'innerJson'),
  );
  ...
  const [chosenIcon, setChosenIcon] = React.useState(
    getSavedCode(activeSnippet, 'icon') || null,
  );
          ...
          {type === 'function' ? (
            <>
              <StringElement
                ...
                value={snippetName}
                error={
                  errorSnippetName ? { message: t('RequiredField') } : null
                }
              />
