 ...
 */
class ValueFunctionFiller extends Filler {
  constructor() {
  ...
  async fill(schemaObject, objectToFill = {}, options = {}) {
    ...
    await this.handleAllElements(schemaObject, objectToFill, async (item, itemSchema) => {
      ...
      let documentData, user;

      // documentData is what we try to fill at that moment
      documentData = jcopy(objectToFill);
      ...
      try {
        ...
        log.save('value-function-filer-get-user-info-error', error, 'warn');
      }
      ...
      try {
        ...
        log.save('value-function-filer-function-calling-error', error, 'warn');
      }
...
module.exports = ValueFunctionFiller;
