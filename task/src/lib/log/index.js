...
class Log {
  ...
  constructor(logProviders = [], activeProviders = []) {
    // Define singleton.
    if (!Log.singleton) {
      if (!logProviders.every(v => v instanceof LogProvider)) { throw new Error(ERROR_MESSAGE_WRONG_PROVIDER); }
  ...
  addProvider(logProvider) {
    if (!(logProvider instanceof LogProvider)) { throw new Error(ERROR_MESSAGE_WRONG_PROVIDER); }
  ...
  async logRouter(req, res, next) {
    ...
    const xTraceService = req.headers['x-trace-service'] || null;
    ...
    // Save.
    this.save('http-request', { handlingInfo, xTraceService, ...requestMeta });
